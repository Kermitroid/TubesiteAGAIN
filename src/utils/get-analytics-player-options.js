...
const filterDefaultsAndNulls = obj =>
  Object.entries(obj).reduce((filtered, [key, value]) => {
...
const getCloudinaryOptions = (cloudinaryOptions = {}) => ({
  ...
  posterOptionsPublicId:
    cloudinaryOptions.posterOptions && hasConfig(cloudinaryOptions.posterOptions.publicId)
});
...
const getTranscriptOptions = (textTracks = {}) => {
  ...
  return {
    ...
    pacedTextTracks:
      (hasConfig(textTracks) && JSON.stringify(textTracks || {}).includes('"maxWords":')) || null,
    wordHighlight:
      (hasConfig(textTracks) && JSON.stringify(textTracks || {}).includes('"wordHighlight":')) ||
      null,
    transcriptLanguages:
      tracksArr
...
const getSourceOptions = (sourceOptions = {}) => ({
  ...
  shoppableProductsLength:
    sourceOptions.shoppable &&
    sourceOptions.shoppable.products &&
    sourceOptions.shoppable.products.length,
  ...(sourceOptions.info
    ? {
    ...
    ? {
        ...
        ...(sourceOptions.textTracks.options
          ? {
              ...
              styledTextTracksWordHighlightStyle: hasConfig(
                sourceOptions.textTracks.options.wordHighlightStyle
              )
            }
          : {})
      }
...
export const getAnalyticsFromPlayerOptions = playerOptions =>
  filterDefaultsAndNulls({
    ...
    controlBar:
      JSON.stringify(playerOptions.controlBar) !== JSON.stringify(defaults.controlBar) &&
      JSON.stringify(playerOptions.controlBar),
