...
import { absolutePathForFilm, absolutePathForFilmImage } from '@/app/paths';
import {
  FUJIFILM_SIMULATION_FORM_INPUT_OPTIONS,
  labelForFujifilmSimulation
} from '@/platforms/fujifilm/simulation';
import { deparameterize, formatCount, formatCountDescriptive } from '@/utility/string';
...
export const sortFilmsWithCount = (a: FilmWithMeta, b: FilmWithMeta) => {
...
export const titleForFilm = (
  ...
  [labelForFilm(film).large, photoQuantityText(explicitCount ?? photos.length, appText)].join(' ');

export const shareTextForFilm = (film: string, appText: AppTextState) =>
...
export const generateMetaForFilm = (
  ...
  description: descriptionForFilmPhotos(photos, appText, true, explicitCount, explicitDateRange),
  images: absolutePathForFilmImage(film)
});

export const photoHasFilmData = (photo: Photo) => Boolean(photo.film);
...
export const convertFilmsForForm = (
  ...
  const films: AnnotatedTag[] = includeAllFujifilmSimulations
    ? FUJIFILM_SIMULATION_FORM_INPUT_OPTIONS.map(({ value }) => ({ value }))
    : [];
