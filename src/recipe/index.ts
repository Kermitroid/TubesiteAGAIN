...
import { capitalizeWords, formatCount, formatCountDescriptive } from '@/utility/string';
...
export const formatRecipe = (recipe?: string) => capitalizeWords(recipe?.replaceAll('-', ' '));
...
export const shareTextForRecipe = (recipe: string, appText: AppTextState) =>
...
export const descriptionForRecipePhotos = (
  ...
  descriptionForPhotoSet(photos, appText, undefined, dateBased, explicitCount, explicitDateRange);

export const generateRecipeLines = ({ title, data, film }: RecipeProps, abbreviate?: boolean) => {
  const lines: string[] = [];
  ...
  lines.push(
    abbreviate ? `${whiteBalance} ${whiteBalanceColor}` : `${whiteBalance}: ${whiteBalanceColor}`
  );
  ...
  return title ? [formatRecipe(title).toLocaleUpperCase(), '–', ...lines] : lines;
};

export const generateRecipeText = (...args: Parameters<typeof generateRecipeLines>) =>
...
export const generateMetaForRecipe = (
  ...
  description: descriptionForRecipePhotos(photos, appText, true, explicitCount, explicitDateRange),
  images: absolutePathForRecipeImage(recipe)
});

const photoHasRecipe = (photo?: Photo) => photo?.film && photo?.recipeData;

const getPhotoWithRecipeFromPhotos = (photos: Photo[], preferredPhoto?: Photo) =>
  photoHasRecipe(preferredPhoto) ? preferredPhoto : photos.find(photoHasRecipe);
...
export const formatWhiteBalance = ({ whiteBalance }: FujifilmRecipe) =>
  whiteBalance.type === 'kelvin' && whiteBalance.colorTemperature
    ...
    : whiteBalance.type.replace(/auto./i, '').replaceAll('-', ' ');

export const formatWhiteBalanceColor = ({ whiteBalance: { red, blue } }: FujifilmRecipe) =>
  `R${addSign(red)}/B${addSign(blue)}`;

export const formatGrain = ({ grainEffect }: FujifilmRecipe, abbreviate?: boolean) => {
  const size = abbreviate ? (grainEffect.size === 'small' ? 'SM' : 'LG') : grainEffect.size;
