...
export default function PhotoRecipeOverlay({
  ...
  const renderDataSquare = (value: ReactNode, label?: string, colSpan = 'col-span-4') => (
    <div
      className={clsx(
  ...
  const renderRecipeTitle = (
    <div className="flex items-center gap-1.5 w-full">
      <TbChecklist size={17} className="opacity-80 translate-y-[1px]" />
      <div
        className={clsx(
          'text-[15px] uppercase',
  ...
  return (
    ...
    >
      <div className={clsx('flex items-center gap-2 h-6', 'pl-1.5 pr-0.5')}>
        <div
          className={clsx(
        ...
        >
          {title ? (
            <PhotoRecipe
              recipe={title}
              className={clsx('text-[15px]', '[&>*>*>*]:text-black', 'tracking-wide')}
            />
          ) : (
            renderRecipeTitle
          )}
        ...
        <CopyButton
          label={`${title ? `${formatRecipe(title).toLocaleUpperCase()} recipe` : 'Recipe'}`}
        ...
        <span>
          <LoaderButton
            ...
            onClick={onClose}
            className={clsx('link p-0 m-0', 'text-black/40 active:text-black/75')}
          />
        </span>
      </div>
      <div className="grid grid-cols-12 gap-2">
        ...
        {renderDataSquare(`DR${dynamicRange.development}`, undefined, 'col-span-4 py-0.5')}
        {/* ROW */}
        {renderDataSquare(
        ...
        {renderDataSquare(formatNoiseReduction(data), 'ISO NR', 'col-span-4')}
        {/* ROW */}
        ...
        {renderDataSquare(colorChromeFXBlue?.toLocaleUpperCase() ?? 'N/A', 'FX Blue', 'col-span-6')}
        {/* ROW */}
        {renderDataSquare(formatGrain(data), 'grain', 'col-span-6')}
