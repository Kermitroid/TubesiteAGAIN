...
export default function PhotoUploadWithStatus({
  ...
  useEffect(() => {
    ...
    if (showButton) {
      ...
      return () => {
        setUploadState?.({ hideUploadPanel: false });
      };
    }
  ...
  const uploadStatusText =
    filesLength > 1 ? appText.utility.paginate(fileUploadIndex + 1, filesLength) : undefined;

  return (
    <div
      className={clsx('flex items-center gap-4', isUploading && 'cursor-not-allowed', className)}
    >
      <div className={clsx(showButton ? 'flex' : 'hidden', 'items-center')}>
        <ImageInput
          ref={inputRef}
          id={inputId}
      ...
      {showStatusText && (
        <div className={clsx('flex items-center gap-4 overflow-hidden')}>
          {isUploading && !showButton && (
            <Spinner className="text-dim translate-y-[1px]" color="text" size={14} />
          )}
          {uploadError ? (
