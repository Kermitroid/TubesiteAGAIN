...
export default function PhotoForm({
  ...
  useEffect(() => {
    if (Object.keys(updatedExifData ?? {}).length > 0) {
      ...
      setFormData(currentForm => {
        (Object.entries(updatedExifData ?? {}) as [keyof PhotoFormData, string][]).forEach(
          ([key, value]) => {
  ...
  const shouldHideField = (
    ...
    if (key === 'blurData' && type === 'create' && !BLUR_ENABLED && !shouldDebugImageFallbacks) {
      ...
      return (hideIfEmpty && !formData[key]) || shouldHide?.(formData, changedFormKeys);
    }
  };
  ...
  const onMatchResults = useCallback((didFindMatchingPhotos: boolean) => {
    setFormData(data => ({
      ...data,
      applyRecipeTitleGlobally: didFindMatchingPhotos ? 'true' : 'false'
    }));
  }, []);
  ...
  return (
    <div className="space-y-8 max-w-[38rem] relative">
      ...
      >
            ...
            ([
              ...
              if (!shouldHideField(key, hideIfEmpty, shouldHide)) {
                const fieldProps: ComponentProps<typeof FieldSetWithStatus> = {
                  ...
                  isModified: !hideModificationStatus && changedFormKeys.includes(key),
                  onChange: value => {
                ...
                switch (key) {
                  ...
                  case 'applyRecipeTitleGlobally':
                    return (
                      <ApplyRecipeTitleGloballyCheckbox
                        ...
                        hasRecipeTitleChanged={changedFormKeys.includes('recipeTitle')}
                  ...
                  case 'favorite':
                    return <FieldsetFavs key={key} {...fieldProps} />;
                  case 'hidden':
                    return <FieldsetHidden key={key} {...fieldProps} />;
                  default:
                    return <FieldSetWithStatus key={key} {...fieldProps} />;
                }
              }
            }
          )}
        ...
        {/* Actions */}
        <div className={clsx('flex gap-3 sticky bottom-0', 'pb-4 md:pb-8 mt-12')}>
          <Link className="button" href={type === 'edit' ? PATH_ADMIN_PHOTOS : PATH_ADMIN_UPLOADS}>
            Cancel
          </Link>
