...
type VirtualFields = 'favorite' | 'applyRecipeTitleGlobally' | 'shouldStripGpsData';

export type FormFields = keyof PhotoDbInsert | VirtualFields;

export type PhotoFormData = Record<FormFields, string>;

export type FieldSetType = 'text' | 'email' | 'password' | 'checkbox' | 'textarea' | 'hidden';
...
export const FIELDS_TO_NOT_OVERWRITE_WITH_NULL_DATA_ON_SYNC = Object.entries(FORM_METADATA())
...
export const convertFormDataToPhotoDbInsert = (
  ...
  return {
    ...
    focalLength: photoForm.focalLength ? parseInt(photoForm.focalLength) : undefined,
    ...
    longitude: photoForm.longitude ? parseFloat(photoForm.longitude) : undefined,
    iso: photoForm.iso ? parseInt(photoForm.iso) : undefined,
    exposureTime: photoForm.exposureTime ? parseFloat(photoForm.exposureTime) : undefined,
    exposureCompensation: photoForm.exposureCompensation
      ? parseFloat(photoForm.exposureCompensation)
      : undefined,
    priorityOrder: photoForm.priorityOrder ? parseFloat(photoForm.priorityOrder) : undefined,
