...
export default function InfinitePhotoScroll({
  ...
  const fetcher = useCallback(
    ([_key, size]: [string, number], warmOnly?: boolean) =>
  ...
  const { data, isLoading, isValidating, error, mutate, size, setSize } = useSwrInfinite<Photo[]>(
    keyGenerator,
    fetcher,
    {
  ...
  const renderMoreButton = () => (
    <div ref={buttonContainerRef}>
      <button
        ...
        className={clsx('w-full flex justify-center', isLoadingOrValidating && 'subtle')}
      >
  ...
  return (
    <div className="space-y-4">
      ...
      {!isFinished &&
        (wrapMoreButtonInGrid ? <AppGrid contentMain={renderMoreButton()} /> : renderMoreButton())}
    </div>
  );
}
