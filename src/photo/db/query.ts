import { sql, query, convertArrayToPostgresString } from '@/platforms/postgres';
import {
  PhotoDb,
  PhotoDbInsert,
  translatePhotoId,
...
import { SYNC_QUERY_LIMIT, UPDATED_BEFORE_01, UPDATED_BEFORE_02 } from '../sync';
...
export const renamePhotoRecipeGlobally = (recipe: string, updatedRecipe: string) =>
  safelyQueryPhotos(
    () => sql`
    UPDATE photos
...
export const getUniqueTags = async () =>
  ...
  `.then(
        ({ rows }): Tags =>
          rows.map(({ tag, count, last_modified }) => ({
            tag: tag as string,
...
export const getUniqueCameras = async () =>
  ...
  `.then(
        ({ rows }): Cameras =>
          rows.map(({ make, model, count, last_modified }) => ({
...
export const getUniqueLenses = async () =>
  ...
  `.then(
        ({ rows }): Lenses =>
          rows.map(({ lens_make: make, lens_model: model, count, last_modified }) => ({
...
export const getUniqueRecipes = async () =>
  ...
  `.then(
        ({ rows }): Recipes =>
          rows.map(({ recipe_title, count, last_modified }) => ({
            recipe: recipe_title,
            count: parseInt(count, 10),
            lastModified: last_modified as Date
          }))
...
export const getRecipeTitleForData = async (data: string | object, film: string) =>
...
export const updateAllMatchingRecipeTitles = (title: string, data: string, film: string) =>
  safelyQueryPhotos(
    () => sql`
    UPDATE photos
...
export const getUniqueFilms = async () =>
  ...
  `.then(
        ({ rows }): Films =>
          rows.map(({ film, count, last_modified }) => ({
            film,
            count: parseInt(count, 10),
            lastModified: last_modified as Date
          }))
...
export const getUniqueFocalLengths = async () =>
  ...
  `.then(
        ({ rows }): FocalLengths =>
          rows.map(({ focal_length, count, last_modified }) => ({
...
export const getPhotoIdsAndUpdatedAt = async () =>
  safelyQueryPhotos(
    () =>
      sql`SELECT id, updated_at FROM photos WHERE hidden IS NOT TRUE`.then(({ rows }) =>
        rows.map(({ id, updated_at }) => ({ id: id as string, updatedAt: updated_at as Date }))
      ),
...
const outdatedWhereClauses = [`updated_at < $1`, `(updated_at < $2 AND make = $3)`];
...
const needsAiTextWhereClauses = AI_TEXT_GENERATION_ENABLED
  ? AI_TEXT_AUTO_GENERATED_FIELDS.map(field => {
      switch (field) {
        case 'title':
          return `(title <> '') IS NOT TRUE`;
        case 'caption':
          return `(caption <> '') IS NOT TRUE`;
        case 'tags':
          return `(tags IS NULL OR array_length(tags, 1) = 0)`;
        case 'semantic':
          return `(semantic_description <> '') IS NOT TRUE`;
      }
    })
  : [];

const needsSyncWhereStatement = `WHERE ${outdatedWhereClauses.concat(needsAiTextWhereClauses).join(' OR ')}`;

export const getPhotosInNeedOfSync = () =>
  safelyQueryPhotos(
    () =>
      query(
        `
...
export const getPhotosInNeedOfSyncCount = () =>
  safelyQueryPhotos(
    () =>
      query(
        `
