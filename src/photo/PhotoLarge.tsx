...
export default function PhotoLarge({
  ...
  const { areZoomControlsShown, arePhotosMatted, shouldDebugRecipeOverlays, isUserSignedIn } =
    useAppState();

  const appText = useAppText();

  const { cameraCount, lensCount, tagCounts, recipeCount, filmCount } =
  ...
  const refTriggers = useMemo(() => [refPhotoRecipe, refPhotoFilm], []);
  const { isShowingRecipeOverlay, toggleRecipeOverlay, hideRecipeOverlay } = useRecipeOverlay({
    ref: refRecipe,
    refTriggers
  });
  ...
  const renderPhotoLink = (
    <PhotoLink photo={photo} className="font-bold uppercase grow" prefetch={prefetch} />
  );

  // Restrict width for landscape photos
  // (portrait photos are always height restricted)
  const matteContentWidthForAspectRatio =
    photo.aspectRatio > 3 / 2 + 0.1 ? 'w-[90%]' : photo.aspectRatio >= 1 ? 'w-[80%]' : undefined;
    ...
    >
      ...
      >
        <ImageLarge
          ...
          classNameImage={clsx(arePhotosMatted && 'object-contain w-full h-full')}
      ...
      <div
        className={clsx(
          ...
          !(isShowingRecipeOverlay || shouldDebugRecipeOverlays) && 'pointer-events-none'
        )}
      >
        <AnimatePresence>
  ...
  return (
    <AppGrid
      ...
      contentMain={
        showZoomControls ? (
          <div className={largePhotoContainerClassName}>{renderLargePhoto}</div>
        ) : (
          <Link
          ...
          <MaskedScroll className="sticky top-4 self-start">
            ...
            >
              ...
              <div className="pr-3 md:pr-0">
                <div className="float-end hidden md:block">{renderAdminMenu}</div>
                {hasTitle && (showTitleAsH1 ? <h1>{renderPhotoLink}</h1> : renderPhotoLink)}
                <div className="space-y-baseline">
                  {photo.caption && <div className="uppercase">{photo.caption}</div>}
                  {(showCameraContent ||
                    showLensContent ||
                    showRecipeContent ||
                    showTagsContent) && (
              ...
              >
                <div className="float-end md:hidden">{renderAdminMenu}</div>
                {showExifContent && (
                  <>
                ...
                >
                  ...
                  />
                  <div className={clsx('flex gap-1 translate-y-[0.5px]', 'translate-x-[-2.5px]')}>
                    {showZoomControls && (
                      <LoaderButton
