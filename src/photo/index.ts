...
export const titleForPhoto = (photo: Photo, useDateAsTitle = true, fallback = 'Untitled') => {
  if (photo.title) {
    return photo.title;
...
export const photoLabelForCount = (count: number, appText: AppTextState, _capitalize = true) => {
  const label = count === 1 ? appText.photo.photo : appText.photo.photoPlural;
  return _capitalize ? capitalize(label) : label;
};
...
export const deleteConfirmationTextForPhoto = (photo: Photo, appText: AppTextState) =>
...
export const descriptionForPhotoSet = (
  ...
  dateBased
    ? dateRangeForPhotos(photos, explicitDateRange).description.toLocaleUpperCase()
    : [
        explicitCount ?? photos.length,
        descriptor || photoLabelForCount(explicitCount ?? photos.length, appText, false)
      ].join(' ');

const sortPhotosByDateNonDestructively = (photos: Photo[], order: 'ASC' | 'DESC' = 'DESC') =>
  [...photos].sort((a, b) =>
    order === 'DESC'
...
export const dateRangeForPhotos = (photos: Photo[] = [], explicitDateRange?: PhotoDateRange) => {
  ...
  if (explicitDateRange || photos.length > 0) {
    ...
    descriptionWithSpaces = start === end ? start : `${start} – ${end}`;
  }
...
const photoHasLensData = (photo: Photo) => Boolean(photo.lensModel);

const photoHasRecipeData = (photo: Photo) => Boolean(photo.recipeData);
...
export const shouldShowLensDataForPhoto = (photo: Photo) =>
  SHOW_EXIF_DATA && SHOW_LENSES && photoHasLensData(photo);

export const shouldShowRecipeDataForPhoto = (photo: Photo) =>
  SHOW_EXIF_DATA && SHOW_RECIPES && photoHasRecipeData(photo);

export const shouldShowFilmDataForPhoto = (photo: Photo) =>
  SHOW_EXIF_DATA && SHOW_FILMS && photoHasFilmData(photo);
...
export const isNextImageReadyBasedOnPhotos = async (photos: Photo[]): Promise<boolean> =>
  photos.length > 0 &&
  fetch(
