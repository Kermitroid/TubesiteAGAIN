...
export const createPhotoAction = async (formData: FormData) =>
  runAuthenticatedAdminServerAction(async () => {
    ...
    const photo = await convertFormDataToPhotoDbInsertAndLookupRecipeTitle(formData);
    ...
    (async () => {
      try {
        for (const url of uploadUrls) {
          ...
          const { formDataFromExif, imageResizedBase64, shouldStripGpsData, fileBytes } =
...
export const toggleFavoritePhotoAction = async (photoId: string, shouldRedirect?: boolean) =>
  runAuthenticatedAdminServerAction(async () => {
    ...
    if (photo) {
      const { tags } = photo;
      photo.tags = isPhotoFav(photo) ? tags.filter(tag => !isTagFavs(tag)) : [...tags, TAG_FAVS];
...
export const toggleHidePhotoAction = async (photoId: string, redirectPath?: string) =>
  runAuthenticatedAdminServerAction(async () => {
...
export const getPhotosNeedingRecipeTitleCountAction = async (
  ...
  runAuthenticatedAdminServerAction(
    async () => await getPhotosNeedingRecipeTitleCount(recipeData, film, photoIdToExclude)
  );
  ...
  runAuthenticatedAdminServerAction(async () => {
    ...
    if (photo) {
      ...
      if (formDataFromExif) {
        ...
        const photoFormDbInsert = await convertFormDataToPhotoDbInsertAndLookupRecipeTitle({
          ...formDataFromPhoto,
          ...formDataFromExif,
          ...
          ...(!photo.semanticDescription && { semanticDescription: aiSemanticDescription })
        });
...
export const streamAiImageQueryAction = async (imageBase64: string, query: AiImageQuery) =>
  runAuthenticatedAdminServerAction(async () => {
    ...
    return streamOpenAiImageQuery(imageBase64, getAiImageQuery(query, existingTags));
  });
...
export const getPhotosAction = async (options: GetPhotosOptions, warmOnly?: boolean) => {
  if (warmOnly) {
    return [];
  } else {
...
export const getPhotosCachedAction = async (options: GetPhotosOptions, warmOnly?: boolean) => {
  if (warmOnly) {
    return [];
  } else {
    return areOptionsSensitive(options)
