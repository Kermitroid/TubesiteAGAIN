...
import { getCategoriesWithItemsCount, PhotoSetCategories } from '@/category';
...
export default function PhotoGridSidebar({
  ...
  const { cameras, lenses, tags, films, recipes, focalLengths } = categories;

  const categoriesCount = getCategoriesWithItemsCount(CATEGORY_VISIBILITY, categories);
  ...
  const camerasContent =
    cameras.length > 0 ? (
      <HeaderList
        ...
        icon={<IconCamera size={15} className="translate-x-[0.5px]" />}
  ...
  const lensesContent =
    lenses.length > 0 ? (
      <HeaderList
        ...
        maxItems={maxItemsPerCategory}
        items={lenses.map(({ lensKey, lens, count }) => (
          <PhotoLens
            key={lensKey}
            lens={lens}
            type="text-only"
  ...
  const tagsContent =
    tags.length > 0 ? (
      <HeaderList
        ...
        icon={<IconTag size={14} className="translate-x-[1px] translate-y-[1px]" />}
        maxItems={maxItemsPerCategory}
  ...
  const recipesContent =
    recipes.length > 0 ? (
      <HeaderList
        ...
        icon={<IconRecipe size={16} className="translate-x-[-1px]" />}
        maxItems={maxItemsPerCategory}
  ...
  const photoStatsContent =
    photosCount > 0 ? (
      start ? (
        ...
        <HeaderList key="photo-stats" items={[photoQuantityText(photosCount, appText, false)]} />
      )
    ) : null;
