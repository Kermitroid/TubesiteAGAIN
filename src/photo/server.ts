import { getExtensionFromStorageUrl, getIdFromStorageUrl } from '@/platforms/storage';
...
import { FujifilmRecipe, getFujifilmRecipeFromMakerNote } from '@/platforms/fujifilm/recipe';
import { getRecipeTitleForData, updateAllMatchingRecipeTitles } from './db/query';
...
export const convertFormDataToPhotoDbInsertAndLookupRecipeTitle = async (
  ...
  if (photo.recipeData && !photo.recipeTitle && photo.film) {
    const recipeTitle = await getRecipeTitleForData(photo.recipeData, photo.film);
    if (recipeTitle) {
...
export const propagateRecipeTitleIfNecessary = async (formData: FormData, photo: PhotoDbInsert) => {
  if (
    ...
    await updateAllMatchingRecipeTitles(photo.recipeTitle, photo.recipeData, photo.film);
  }
};
