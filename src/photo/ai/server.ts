...
import { AiAutoGeneratedField, getAiImageQuery, parseTitleAndCaption } from '.';
...
export const generateAiImageQueries = async (
  ...
  try {
    if (imageBase64) {
      if (textFieldsToGenerate.includes('title') && textFieldsToGenerate.includes('caption')) {
        ...
        if (textFieldsToGenerate.includes('title')) {
          title = await generateOpenAiImageQuery(imageBase64, getAiImageQuery('title'), isBatch);
        }
