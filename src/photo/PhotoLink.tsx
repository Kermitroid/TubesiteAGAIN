...
export default function PhotoLink({
  ...
  const linkProps: Omit<ComponentProps<typeof LinkWithStatus>, 'children'> | undefined = photo
    ? {
        ref,
        className,
  ...
  const children = photo ? (_children ?? titleForPhoto(photo)) : _children;

  return photo && linkProps ? (
    loaderType === 'spinner' ? (
      ...
      <LinkWithLoaderBackground {...linkProps} offsetPadding>
        {children}
      </LinkWithLoaderBackground>
    )
  ) : (
    <span className={clsx('text-gray-300 dark:text-gray-700 cursor-default', className)}>
      {children}
    </span>
  );
}
