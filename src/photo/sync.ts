...
import { AI_TEXT_AUTO_GENERATED_FIELDS, AI_TEXT_GENERATION_ENABLED } from '@/app/config';
...
const isPhotoOutdated = (photo: PhotoDb) =>
  ...
  (photo.updatedAt < UPDATED_BEFORE_02 && photo.make === MAKE_FUJIFILM);
...
export const photoNeedsToBeSynced = (photo: Photo) =>
  photo.syncStatus.isOutdated || photo.syncStatus.missingAiTextFields.length > 0;
...
export const getPhotosSyncStatusText = (photos: Photo[]) => {
  ...
  const photosCountOutdated = photos.filter(photo => photo.syncStatus.isOutdated).length;
