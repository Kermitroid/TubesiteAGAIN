...
import { Photo, downloadFileNameForPhoto, getNextPhoto, getPreviousPhoto } from '@/photo';
...
import { ALLOW_PUBLIC_DOWNLOADS, SHOW_KEYBOARD_SHORTCUT_TOOLTIPS } from '@/app/config';
...
export default function PhotoPrevNextActions({
  ...
  const photoTitle = photo ? (photo.title ? `'${photo.title}'` : 'photo') : undefined;
  ...
  const toggleHidden = useCallback(() => {
    if (photo?.id) {
      return toggleHidePhotoAction(photo.id);
    }
  }, [photo?.id]);
  ...
  const pathNext = nextPhoto ? pathForPhoto({ photo: nextPhoto, ...categories }) : undefined;
      ...
      if (e.metaKey) {
        ...
        switch (e.key.toUpperCase()) {
          ...
          case KEY_COMMANDS.download:
            if ((isUserSignedIn || ALLOW_PUBLIC_DOWNLOADS) && downloadUrl && downloadFileName) {
      ...
      >
        ...
        >
          ...
          >
            ...
            <span className="hidden sm:inline-block uppercase">{appText.nav.prevShort}</span>
          </PhotoLink>
        </Tooltip>
        ...
        >
          ...
          >
            ...
            <span className="hidden sm:inline-block uppercase">{appText.nav.nextShort}</span>
          </PhotoLink>
        </Tooltip>
      </div>
    </div>
