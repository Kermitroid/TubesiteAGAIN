...
export const vercelBlobPut = (file: Buffer, fileName: string): Promise<string> =>
  put(fileName, file, { access: 'public' }).then(({ url }) => url);
...
export const vercelBlobCopy = (
  ...
  copy(sourceUrl, destinationFileName, { access: 'public', addRandomSuffix }).then(
    ({ url }) => url
  );

export const vercelBlobDelete = (fileName: string) => del(fileName);

export const vercelBlobList = (prefix: string): Promise<StorageListResponse> =>
  list({ prefix }).then(({ blobs }) =>
    blobs.map(({ url, uploadedAt, size }) => ({
      url,
