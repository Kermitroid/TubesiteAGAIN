...
const FUJIFILM_SIMULATION_LABELS: Record<FujifilmSimulation, FujifilmSimulationLabel> = {
  monochrome: {
    small: 'Monochrome',
    medium: 'Monochrome',
    large: 'Monochrome'
...
export const FUJIFILM_SIMULATION_FORM_INPUT_OPTIONS = Object.entries(FUJIFILM_SIMULATION_LABELS)
  .map(
    ([value, label]) =>
      ({ value, label: label.large }) as { value: FujifilmSimulation; label: string }
  )
  .sort((a, b) => a.label.localeCompare(b.label));

const ALL_POSSIBLE_FUJIFILM_SIMULATION_LABELS = Object.values(FUJIFILM_SIMULATION_LABELS).flatMap(
  ({ small, medium, large }) => [
...
export const isStringFujifilmSimulation = (film?: string) =>
  film !== undefined && Object.keys(FUJIFILM_SIMULATION_LABELS).includes(film);
