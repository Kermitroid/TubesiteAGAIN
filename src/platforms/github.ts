import { TEMPLATE_REPO_OWNER, TEMPLATE_REPO_NAME, TEMPLATE_REPO_BRANCH } from '@/app/config';
...
const fetchGitHub = async (url: string, cacheRequest = true) => {
...
export const getGitHubUrlOwner = ({ owner = TEMPLATE_REPO_OWNER }: RepoParams = {}) =>
  `https://github.com/${owner}`;

export const getGitHubUrlRepo = ({
  owner = TEMPLATE_REPO_OWNER,
  repo = TEMPLATE_REPO_NAME
}: RepoParams = {}) => `${getGitHubUrlOwner({ owner })}/${repo}`;

export const getGitHubUrlBranch = ({
  owner = TEMPLATE_REPO_OWNER,
  repo = TEMPLATE_REPO_NAME,
  branch = DEFAULT_BRANCH
}: RepoParams = {}) => `${getGitHubUrlRepo({ owner, repo })}/tree/${branch}`;
...
export const getGitHubUrlCommit = ({
  ...
  commit ? `${getGitHubUrlRepo({ owner, repo })}/commit/${commit}` : undefined;

export const getGitHubUrlCompare = ({ owner, repo, branch = DEFAULT_BRANCH }: RepoParams = {}) =>
...
const getGitHubApiRepoUrl = ({
  owner = TEMPLATE_REPO_OWNER,
  repo = TEMPLATE_REPO_NAME
}: RepoParams = {}) => `https://api.github.com/repos/${owner}/${repo}`;

const getGitHubApiCommitsUrl = (params?: RepoParams) =>
  `${getGitHubApiRepoUrl(params)}/commits/${params?.branch || DEFAULT_BRANCH}`;

const getGitHubApiForksUrl = (params?: RepoParams) => `${getGitHubApiRepoUrl(params)}/forks`;

const getGitHubApiCompareToRepoUrl = ({ owner, repo, branch = DEFAULT_BRANCH }: RepoParams = {}) =>
...
const getIsRepoForkedFromBase = async (params: RepoParams) => {
  ...
  return (
    Boolean(data.fork) && data.source?.full_name === `${TEMPLATE_REPO_OWNER}/${TEMPLATE_REPO_NAME}`
  );
};
...
const isRepoBaseRepo = ({ owner, repo }: RepoParams) =>
  owner?.toLowerCase() === TEMPLATE_REPO_OWNER && repo?.toLowerCase() === TEMPLATE_REPO_NAME;
...
export const getGitHubMeta = async (params: RepoParams) => {
  ...
  try {
    ...
    isBehind = behindBy === undefined ? undefined : behindBy > 0;
  } catch (error) {
    didError = true;
