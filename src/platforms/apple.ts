...
export const isCameraMakeApple = (make?: string) => make?.toLocaleLowerCase() === MAKE_APPLE;

export const isCameraApple = ({ make }: Camera) => isCameraMakeApple(make);

export const isLensMakeApple = (make?: string) => make?.toLocaleLowerCase() === MAKE_APPLE;

export const isLensApple = ({ make, model }: Lens) =>
  isLensMakeApple(make) || /^iPhone [0-9a-z]{1,3}/i.test(model);

export const formatAppleLensText = (model: string, includeDeviceName?: boolean) => {
