...
class CloudinaryContext {
  constructor(player, options = {}) {
    ...
    this.source = (source, options = {}) => {
      ...
      if (recommendations && recommendations.length) {
        ...
        if (options.recommendationOptions) {
          ({ disableAutoShow, itemBuilder } = sliceAndUnsetProperties(
    ...
    this.buildSource = (publicId, options = {}) => {
      ...
      options.cloudinaryConfig = extendCloudinaryConfig(
      ...
      options.queryParams = Object.assign(
        options.queryParams || {},
        options.usageReport ? { _s: `vp-${VERSION}` } : {}
      );
    ...
    const setRecommendations = (
      recommendations,
      { disableAutoShow = false, itemBuilder = null }
    ) => {
      unsetRecommendations();
      ...
      itemBuilder =
        itemBuilder ||
        (source => ({
          source: source instanceof VideoSource ? source : this.buildSource(source),
    ...
    const refresh = () => {
      ...
      const posterOptions = Object.assign(
        {},
        this.player.cloudinary.posterOptions(),
        src.getInitOptions().poster
      );
    ...
    const testCanPlayTypeAndTypeSupported = codec => {
      ...
      return (
        v.canPlayType(codec) || ('MediaSource' in window && MediaSource.isTypeSupported(codec))
      );
    };
    ...
    const posterOptionsForCurrent = () => {
      ...
      if (
        ...
        !isKeyInTransformation(opts.transformation, 'width') &&
        !isKeyInTransformation(opts.transformation, 'height')
      ) {
