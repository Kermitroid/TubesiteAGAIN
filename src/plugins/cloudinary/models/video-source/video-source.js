...
import { formatToMimeTypeAndTransformation, hasCodec, normalizeFormat } from './video-source.utils';
  ...
  generateSources() {
    ...
    const srcs = this.sourceTypes().map(sourceType => {
      ...
      if (isAdaptive) {
        ...
        const hasStreamingProfile = JSON.stringify(opts.transformation || {}).includes(
          '"streaming_profile":'
        );
