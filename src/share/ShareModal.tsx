...
export default function ShareModal({
  ...
  const { setShareModalProps, setShouldRespondToKeyboardCommands } = useAppState();
  ...
  const renderIcon = (icon: ReactNode, action: () => void, embedded?: boolean) => (
    <div
      className={clsx(
        'py-2.5 px-3',
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <MaskedScroll
              ...
              scrollToEndOnMount
            >
              <div className="whitespace-nowrap px-2 sm:px-3">{shortenUrl(pathShare)}</div>
            </MaskedScroll>
            {renderIcon(
              <BiCopy size={18} />,
              () => {
