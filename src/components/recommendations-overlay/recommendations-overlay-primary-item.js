...
class RecommendationsOverlayPrimaryItem extends RecommendationsOverlayItem {
  setItem(item) {
    ...
    if (info.description) {
      ...
      const description =
        info.description.length > descLength
          ? info.description.substring(0, descLength) + '...'
          : info.description;
  ...
  createEl() {
    ...
    this.content = dom.createEl('div', {
      className:
        'vjs-recommendations-overlay-item-info vjs-recommendations-overlay-item-primary-content'
    });
