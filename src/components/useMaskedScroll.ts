...
export default function useMaskedScroll({
  ...
  const updateMask = useCallback(() => {
    ...
    if (ref) {
      const start = isVertical ? ref.scrollTop < 1 : ref.scrollLeft < 1;
      ...
      ref.style.setProperty(
        CSS_VAR_MASK_COLOR_START,
        start ? 'rgba(0, 0, 0, 1)' : 'rgba(0, 0, 0, 0)'
      );
      ref.style.setProperty(CSS_VAR_MASK_COLOR_END, end ? 'rgba(0, 0, 0, 1)' : 'rgba(0, 0, 0, 0)');
    }
  ...
  useEffect(() => {
    ...
    if (scrollToEndOnMount && ref && contentRect) {
      ref.scrollTo(isVertical ? { top: contentRect.height } : { left: contentRect.width });
    }
