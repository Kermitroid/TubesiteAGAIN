...
export default function ImageWithFallback({
  ...
  useEffect(() => {
    setWasCached(Boolean(imgRef.current?.complete) && (imgRef.current?.naturalWidth ?? 0) > 0);
  }, []);
    ...
    <div className={clsx('flex relative', className)}>
      ...
      <div
        className={clsx(
          ...
          'overflow-hidden',
          (showFallback || shouldDebugImageFallbacks) && 'transition-opacity duration-300 ease-in',
        ...
        {BLUR_ENABLED && blurDataURL ? (
          <img
            {...{
              ...props,
              src: blurDataURL,
              className: clsx(getBlurClass(), classNameImage)
            }}
          />
        ) : (
          <div className={clsx('w-full h-full', 'bg-gray-100/50 dark:bg-gray-900/50')} />
        )}
      </div>
    </div>
  );
