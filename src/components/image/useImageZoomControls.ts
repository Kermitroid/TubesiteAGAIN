...
export default function useImageZoomControls({
  ...
  useEffect(() => {
    if (isEnabled) {
      const imageRef = selectImageElement?.(refImageContainer.current) ?? refImageContainer.current;
      if (imageRef) {
  ...
  }, [isEnabled, refImageContainer, selectImageElement, setShouldRespondToKeyboardCommands]);
