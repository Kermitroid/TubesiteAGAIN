...
export default function ZoomControls({
  ...
  const { open, reset, zoomTo, zoomLevel, refViewerContainer } = useImageZoomControls({
    refImageContainer,
    ...props
  });
  ...
  return (
    <div ref={refImageContainer} className={clsx('h-full', props.isEnabled && 'cursor-zoom-in')}>
      {children}
      {refViewerContainer.current ? createPortal(button, refViewerContainer.current) : null}
    </div>
  );
}
