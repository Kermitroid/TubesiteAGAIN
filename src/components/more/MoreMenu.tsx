  ...
  return (
    <DropdownMenu.Root open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DropdownMenu.Portal>
        ...
        >
          {header && (
            <div className={clsx('px-3 pt-3 pb-2 text-dim uppercase', 'text-sm tracking-wide')}>
              {header}
            </div>
          )}
          <div className="divide-y divide-medium">
            {sections.map((section, index) => (
              <div
                key={index}
                className={clsx('[&:not(:first-child)]:pt-1', '[&:not(:last-child)]:pb-1')}
              >
                {section.map(props => (
                  <div key={props.label} className="px-1">
                    <MoreMenuItem dismissMenu={dismissMenu} {...props} />
                  </div>
                ))}
              </div>
            ))}
