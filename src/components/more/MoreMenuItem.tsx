...
export default function MoreMenuItem({
  ...
  return (
    ...
    >
      ...
      >
        <span>{labelComplex ?? label}</span>
        {annotation && <span className="text-dim ml-3">{annotation}</span>}
      </LoaderButton>
      {keyCommand && (
        <KeyCommand modifier={keyCommandModifier} className="hidden! sm:inline-flex!">
          {keyCommand}
        </KeyCommand>
      )}
    </DropdownMenu.Item>
