...
export default function TagInput({
  ...
  const hasReachedLimit = useMemo(
    () => limit !== undefined && selectedOptions.length >= limit && !behaveAsDropdown,
  ...
  const addOptions = useCallback(
    (options: (string | undefined)[]) => {
      ...
      if (behaveAsDropdown || (limit !== undefined && limit - 1 >= selectedOptions.length)) {
        hideMenu(true);
      } else {
    ...
    (value: string) => {
      ...
      return (
        <>
          ...
          {icon && <span className="text-medium">{icon}</span>}
        </>
      );
    },
    [options, defaultIcon]
            ...
            {optionsFiltered.map(({ value, annotation, annotationAria }, index) => (
              <div
                ...
                className={clsx(
                  ...
                  !hasReachedLimit && 'active:bg-gray-50 dark:active:bg-gray-900',
              ...
              >
                ...
                {annotation && (
                  <span className="whitespace-nowrap text-dim text-sm" aria-label={annotationAria}>
