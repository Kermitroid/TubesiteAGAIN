...
export default function ResponsiveDate({
  ...
  const titleDateFormatted = formatDate({ date, timezone }).toLocaleUpperCase();

  const title = titleLabel ? `${titleLabel}: ${titleDateFormatted}` : titleDateFormatted;
  ...
  return (
    ...
    >
      {/* Small */}
      <span className={clsx('xs:hidden', contentClass)} aria-hidden>
        {formatDate({ ...formatDateProps, length: 'short' })}
      </span>
      {/* Medium */}
      <span className={clsx('hidden xs:inline sm:hidden', contentClass)} aria-hidden>
        {formatDate({ ...formatDateProps, length: 'medium' })}
      </span>
      {/* Large */}
      <span className={clsx('hidden sm:inline', contentClass)}>{formatDate(formatDateProps)}</span>
    </span>
  );
}
