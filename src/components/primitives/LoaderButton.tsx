...
import { ButtonHTMLAttributes, ComponentProps, ReactNode, RefObject } from 'react';
  ...
  const button = (
    <button
      ...
      className={clsx(
        'font-mono',
        ...(styleAs !== 'button'
          ? ['link h-4 active:text-medium', 'disabled:bg-transparent!']
          : ['h-9']),
        ...
        >
          {isLoading ? (
            <Spinner
              size={14}
              color={spinnerColor}
              className={clsx('translate-y-[1px]', spinnerClassName)}
            />
          ) : (
            icon
          )}
