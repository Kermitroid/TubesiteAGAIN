...
export default function TooltipPrimitive({
  ...
  const classNameTrigger = clsx('cursor-default inline-flex', classNameTriggerProp);

  const content = keyCommand ? (
    <div className="-mr-0.5 whitespace-nowrap">
      {contentProp} <KeyCommand {...{ modifier: keyCommandModifier }}>{keyCommand}</KeyCommand>
    </div>
  ) : (
    contentProp
  );
      ...
      <Tooltip.Root open={includeButton ? isOpen : undefined}>
        <Tooltip.Trigger asChild>
          {includeButton ? (
            ...
            <span className={classNameTrigger} onClick={blurActiveElement}>
              {children}
            </span>
          )}
        </Tooltip.Trigger>
        <Tooltip.Portal>
          ...
          >
            {content && <MenuSurface {...{ color, className }}>{content}</MenuSurface>}
          </Tooltip.Content>
        </Tooltip.Portal>
      </Tooltip.Root>
    </Tooltip.Provider>
