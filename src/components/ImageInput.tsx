...
export default function ImageInput({
  ...
  const {
    uploadState: { isUploading, image, filesLength, fileUploadIndex },
    setUploadState,
    resetUploadState
  } = useAppState();
        ...
        >
          {showButton && (
            <ProgressButton
              ...
              icon={
                <FiUploadCloud size={18} className="translate-x-[-0.5px] translate-y-[0.5px]" />
              }
              ...
              if (files && files.length > 0) {
                ...
                for (let i = 0; i < files.length; i++) {
                  ...
                  if ((shouldResize || isPng) && canvas && ctx) {
                    ...
                    let orientation = (await getOrientation(file).catch(() => 1)) ?? 1;
