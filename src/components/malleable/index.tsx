...
export type ValidElementTypes = 'p' | 'div' | 'span' | 'h1' | 'h2' | 'h3' | 'h4' | 'pre';
...
export default function Malleable({
  ...
  if (isActive) {
    return (
      <Suspense
        fallback={
          <As ref={contentRef as any} className={className + ' text-muted'} id={id}>
            {editedChildren}
          </As>
        }
      >
        <SlateWrapper As={As} id={id} className={className} initialText={initialText} />
      </Suspense>
    );
  }
  return (
