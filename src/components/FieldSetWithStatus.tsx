...
export default function FieldSetWithStatus({
  ...
  const inputProps: InputHTMLAttributes<HTMLInputElement> = {
    ...
    type,
    value,
    checked: type === 'checkbox' ? (value === 'true' ? true : false) : undefined,
    placeholder,
    onChange: e =>
      onChange?.(
        type === 'checkbox' ? (e.target.value === 'true' ? 'false' : 'true') : e.target.value
      ),
    spellCheck,
    autoComplete: 'off',
    ...
    className: clsx(
      ...
      type === 'checkbox' && (readOnly || pending || loading) && 'opacity-50 cursor-not-allowed',
      Boolean(error) && 'error'
    )
  };
      ...
      {!hideLabel && (
        ...
        >
          <span className="inline-flex items-center gap-x-1.5">
            {icon && <span className="inline-flex items-center justify-center w-4">{icon}</span>}
            {label}
          </span>
          {note && !error && (
            <ResponsiveText
          ...
          {isModified && !error && (
            <span
              className={clsx('text-main font-medium text-[0.9rem]', ' -ml-1.5 translate-y-[-1px]')}
            >
              *
            </span>
          )}
      ...
      <div className="flex gap-2">
        {selectOptions ? (
          ...
          <textarea
            ...
            autoCapitalize={!capitalize ? 'off' : undefined}
            className={clsx('w-full h-24 resize-none', Boolean(error) && 'error')}
          />
        ) : type === 'checkbox' ? (
          <Checkbox
            ref={inputRef}
            accessory={loading && <Spinner className="translate-y-[0.5px]" />}
            {...inputProps}
          />
        ) : (
          <input ref={inputRef} {...inputProps} />
