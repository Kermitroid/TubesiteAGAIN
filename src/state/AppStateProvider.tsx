...
import { HIGH_DENSITY_GRID, IS_DEVELOPMENT, MATTE_PHOTOS, SHOW_ZOOM_CONTROLS } from '@/app/config';
import { ShareModalProps } from '@/share';
import { storeTimezoneCookie } from '@/utility/timezone';
import { AdminData, getAdminDataAction } from '@/admin/actions';
import { storeAuthEmailCookie, clearAuthEmailCookie, getAuthEmailCookie } from '@/auth';
...
export default function AppStateProvider({ children }: { children: ReactNode }) {
  ...
  const [nextPhotoAnimation, _setNextPhotoAnimation] = useState<AnimationConfig>();
  ...
  const [nextPhotoAnimationId, setNextPhotoAnimationId] = useState<string>();
  const getNextPhotoAnimationId = useCallback(() => {
    const id = nanoid();
    setNextPhotoAnimationId(id);
    return id;
  }, []);
  const clearNextPhotoAnimation = useCallback(
    (id?: string) => {
  ...
  const [recipeModalProps, setRecipeModalProps] = useState<RecipeProps>();
  // AUTH
  ...
  const [areZoomControlsShown, setAreZoomControlsShown] = useState(SHOW_ZOOM_CONTROLS);
  ...
  const [shouldDebugInsights, setShouldDebugInsights] = useState(IS_DEVELOPMENT);
  const [shouldDebugRecipeOverlays, setShouldDebugRecipeOverlays] = useState(false);
  ...
  } = useSWR(isUserSignedIn ? 'getAdminData' : null, getAdminDataAction);
