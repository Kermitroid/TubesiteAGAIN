...
export default function AdminPhotosSyncClient({
  ...
  return (
    <AdminChildPage
      ...
      breadcrumb={<ResponsiveText shortText="Updates">Updates ({photos.length})</ResponsiveText>}
      accessory={
        <ProgressButton
          primary
            ...
            if (
              window.confirm(
                [
                  ...
                  photos.length === 1 ? '1 photo?' : `all ${photos.length} photos?`,
              ...
              while (photoIdsToSync.current.length > 0) {
                const photoIds = photoIdsToSync.current.slice(0, SYNC_BATCH_SIZE_MAX);
                ...
                await syncPhotosAction(photoIds)
                  .then(() => {
                    ...
                    setProgress((photos.length - photoIdsToSync.current.length) / photos.length);
                    router.refresh();
                  })
                  .catch(e => {
                    errorRef.current = e;
        ...
        >
          {arePhotoIdsSyncing ? 'Syncing ...' : 'Sync All'}
        </ProgressButton>
      }
    >
      <div className="space-y-6">
        {error && (
          <ErrorNote>
            <span className="font-bold">Issue syncing:</span> {error.message}
          </ErrorNote>
        )}
        <Note color="blue" icon={<LiaBroomSolid size={18} />}>
          <div className="space-y-1.5">
            <div className="font-bold">Photo updates: {statusText}</div>
            Sync to capture new EXIF fields, improve blur data, use AI to generate missing text (if
            configured)
          </div>
        </Note>
