...
export default function AdminAppMenu({
  ...
  const sectionUpload: ComponentProps<typeof MoreMenuItem>[] = useMemo(
    () => [
      {
        label: appText.admin.uploadPhotos,
        icon: <IconUpload size={15} className="translate-x-[0.5px] translate-y-[0.5px]" />,
        annotation: isLoadingAdminData && <Spinner className="translate-y-[1.5px]" />,
        action: startUpload
      }
    ],
  ...
  const sectionMain: ComponentProps<typeof MoreMenuItem>[] = useMemo(() => {
    ...
    if (uploadsCount) {
      items.push({
        ...
        icon: <IconFolder size={16} className="translate-x-[1px] translate-y-[1px]" />,
    ...
    if (photosCountNeedSync) {
      items.push({
        ...
        annotation: (
          <>
            <span className="mr-3">{photosCountNeedSync}</span>
        ...
        icon: <IconBroom size={18} className="translate-y-[-0.5px]" />,
    ...
    if (photosCountTotal) {
      items.push({
        ...
        icon: <IconPhoto size={15} className="translate-x-[-0.5px] translate-y-[1px]" />,
    ...
    if (recipesCount) {
      items.push({
        ...
        icon: <IconRecipe size={17} className="translate-x-[-0.5px] translate-y-[1px]" />,
        href: PATH_ADMIN_RECIPES
      });
    }
    if (photosCountTotal) {
      items.push({
        label: isSelecting ? appText.admin.batchExitEdit : appText.admin.batchEditShort,
        icon: isSelecting ? (
          <IoCloseSharp size={18} className="translate-x-[-1px] translate-y-[1px]" />
        ) : (
          <IoMdCheckboxOutline size={16} className="translate-x-[-0.5px]" />
        ),
    ...
    items.push({
      label: showAppInsightsLink ? appText.admin.appInsights : appText.admin.appConfig,
      icon: <AdminAppInfoIcon size="small" className="translate-x-[-0.5px] translate-y-[0.5px]" />,
      href: showAppInsightsLink ? PATH_ADMIN_INSIGHTS : PATH_ADMIN_CONFIGURATION
    });
  ...
  const sectionSignOut: ComponentProps<typeof MoreMenuItem>[] = useMemo(
    () => [
      {
  ...
  return (
    <MoreMenu
      {...{ isOpen, setIsOpen }}
      icon={
        <div className={clsx('w-[28px] h-[28px]', 'overflow-hidden')}>
          <div
            className={clsx(
      ...
      className={clsx('outline-medium', className)}
