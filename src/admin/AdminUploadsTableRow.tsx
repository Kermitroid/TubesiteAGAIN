...
export default function AdminUploadsTableRow({
  ...
  useEffect(() => {
    if (status === 'adding' && !isElementEntirelyInViewport(ref.current)) {
      window.scrollTo({
  ...
  return (
    ...
    >
      <div className={clsx('w-[50%] sm:w-auto shrink-0 bg-dim', 'transition-transform')}>
        <ImageMedium title={getIdFromStorageUrl(url)} src={url} alt={url} aspectRatio={3.0 / 2.0} />
      </div>
      <div
        className={clsx(
      ...
      >
        <div className="flex flex-col gap-0.5 h-full">
          <div className="truncate font-medium">
            {uploadedAt ? <ResponsiveDate date={uploadedAt} /> : '—'}
          </div>
          ...
          {isAdding || isComplete ? (
            <>
              {status === 'added' ? (
                ...
                status === 'adding' && <Spinner size={19} className="translate-y-[2px]" />
              )}
            </>
          ) : (
            <>
