...
import { getGitHubMetaForCurrentApp, indicatorStatusForSignificantInsights } from './insights';
...
export const getAdminDataAction = async () =>
  runAuthenticatedAdminServerAction(async () => {
    ...
    const photosCountTotal =
      photosCount !== undefined && photosCountHidden !== undefined
...
export const testConnectionsAction = async () =>
  runAuthenticatedAdminServerAction(async () => {
    const { hasDatabase, hasStorageProvider, hasRedisStorage, isAiTextGenerationEnabled } =
      APP_CONFIGURATION;

    const [databaseError, storageError, redisError, aiError] = await Promise.all([
