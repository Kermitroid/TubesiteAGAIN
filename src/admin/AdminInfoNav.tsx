...
export default function AdminInfoPage({ includeInsights }: { includeInsights: boolean }) {
  const pathname = usePathname();

  const pages = includeInsights ? ADMIN_INFO_PAGES : ADMIN_INFO_PAGE_WITHOUT_INSIGHTS;
  ...
  return (
    <div className="flex items-center gap-4 min-h-9">
      <div className={clsx('grow -translate-x-1', 'flex items-center gap-1.5 md:gap-3')}>
        {pages.map(({ title, titleShort, path }) => (
          <LinkWithLoaderBackground
            key={path}
            href={path}
            className={clsx(
              'relative',
              hasMultiplePages ? (pathname === path ? 'font-medium' : 'text-dim') : undefined,
          ...
          >
            ...
            {title === 'App Insights' && insightsIndicatorStatus && (
              <InsightsIndicatorDot size="small" top={4} right={-2} />
            )}
          </LinkWithLoaderBackground>
        ))}
      </div>
