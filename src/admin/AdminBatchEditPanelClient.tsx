...
export default function AdminBatchEditPanelClient({ uniqueTags }: { uniqueTags: Tags }) {
  ...
  const photosText = photoQuantityText(selectedPhotoIds?.length ?? 0, appText, false, false);

  const isFormDisabled = isPerformingSelectEdit || selectedPhotoIds?.length === 0;

  const renderPhotoCTA =
    selectedPhotoIds?.length === 0 ? (
      <>
        <FaArrowDown />
        <ResponsiveText shortText="Select below">Select photos below</ResponsiveText>
      </>
    ) : (
  ...
  const shouldShowPanel =
    isUserSignedIn && pathname === PATH_GRID_INFERRED && selectedPhotoIds !== undefined;
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            {isInTagMode ? (
              ...
              <div className="text-base flex gap-2 items-center">{renderPhotoCTA}</div>
            )}
          </Note>
