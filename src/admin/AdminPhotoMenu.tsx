...
import { getPathComponents, PATH_ROOT, pathForAdminPhotoEdit, pathForTag } from '@/app/paths';
import {
  deletePhotoAction,
  syncPhotoAction,
...
export default function AdminPhotoMenu({
  ...
  const sectionMain = useMemo(() => {
    const items: ComponentProps<typeof MoreMenuItem>[] = [
      {
        ...
        icon: <IconEdit size={15} className="translate-x-[0.5px]" />,
    ...
    if (includeFavorite) {
      items.push({
        ...
        ...(showKeyCommands && {
          keyCommand: isFav ? KEY_COMMANDS.unfavorite : KEY_COMMANDS.favorite
        })
      });
    }
    items.push({
      ...
      action: () =>
        toggleHidePhotoAction(photo.id, redirectPathOnHideToggle).then(() =>
          revalidatePhoto?.(photo.id)
        ),
      ...(showKeyCommands && {
    ...
    items.push({
      ...
      icon: <MdOutlineFileDownload size={17} className="translate-x-[-1px]" />,
      href: photo.url,
    ...
    items.push({
      ...
      icon: <IconGrSync className="translate-x-[-1px] translate-y-[0.5px]" />,
      action: () => syncPhotoAction(photo.id).then(() => revalidatePhoto?.(photo.id)),
  ...
  const sectionDelete: ComponentProps<typeof MoreMenuItem>[] = useMemo(
    () => [
      {
        ...
        icon: <BiTrash size={15} className="translate-x-[-1px]" />,
    ...
    [appText, photo, showKeyCommands, revalidatePhoto, shouldRedirectDelete, registerAdminUpdate]
  );

  const sections = useMemo(() => [sectionMain, sectionDelete], [sectionMain, sectionDelete]);
