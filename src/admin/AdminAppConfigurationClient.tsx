...
export default function AdminAppConfigurationClient({
  ...
  const renderSubStatusWithEnvVar = (
    type: ComponentProps<typeof StatusIcon>['type'],
    variable: string
  ) => renderSubStatus(type, renderEnvVars([variable]), 'translate-y-[7px]');
  ...
  return (
    <ScoreCardContainer>
      <ChecklistGroup title="Storage" icon={<BiData size={16} />}>
        <ChecklistRow
          title={
            hasDatabase && isAnalyzingConfiguration
        ...
        >
          ...
          {hasDatabase &&
            ...
            renderSubStatus(
              'checked',
              <>
                Postgres-compatible: connected (SSL {isPostgresSslEnabled ? 'enabled' : 'disabled'})
              </>
            )}
        </ChecklistRow>
        <ChecklistRow
      ...
      <ChecklistGroup title="Authentication" icon={<BiLockAlt size={16} />}>
        <ChecklistRow
          title={!hasAuthSecret && isAnalyzingConfiguration ? 'Generating secret' : 'Setup auth'}
      ...
      <ChecklistGroup title="Content" icon={<BiPencil size={16} />}>
        <ChecklistRow title={`Configure language: ${locale}`} status={hasLocale} optional>
          Store in environment variable (check README for{' '}
          <AdminLink href="https://github.com/sambecker/exif-photo-blog?tab=readme-ov-file#supported-languages">
            supported languages
          </AdminLink>
          ):
          {renderEnvVars(['NEXT_PUBLIC_LOCALE'])}
        </ChecklistRow>
        <ChecklistRow title="Configure domain" status={hasDomain}>
          Store in environment variable (used in explicit share urls, seen in nav if no title is
          defined):
          {renderEnvVars(['NEXT_PUBLIC_DOMAIN'])}
        </ChecklistRow>
        <ChecklistRow title="Meta title" status={isMetaTitleConfigured} showWarning>
          Store in environment variable (seen in search results and browser tab):
          {renderEnvVars(['NEXT_PUBLIC_META_TITLE'])}
        </ChecklistRow>
        {!simplifiedView && (
          <>
            <ChecklistRow title="Meta description" status={isMetaDescriptionConfigured} optional>
              Store in environment variable (seen in search results):
              {renderEnvVars(['NEXT_PUBLIC_META_DESCRIPTION'])}
            </ChecklistRow>
            <ChecklistRow title="Nav title" status={hasNavTitle} optional>
              Store in environment variable (replaces domain in nav):
              {renderEnvVars(['NEXT_PUBLIC_NAV_TITLE'])}
            </ChecklistRow>
            <ChecklistRow title="Nav caption" status={hasNavCaption} optional>
              Store in environment variable (seen in nav, under title):
              {renderEnvVars(['NEXT_PUBLIC_NAV_CAPTION'])}
            </ChecklistRow>
            <ChecklistRow title="Page about" status={hasPageAbout} optional>
      ...
      {!simplifiedView && (
        <>
          <ChecklistGroup title="AI text generation" titleShort="AI" icon={<HiSparkles />} optional>
            <ChecklistRow
              title={
                isAiTextGenerationEnabled && isAnalyzingConfiguration
            ...
            >
              ...
              Store your OpenAI secret key in order to enable AI-generated text descriptions and
              optionally leverage an invisible field called {'"Semantic Description"'} used to
              support CMD-K search and improve accessibility:
            ...
            >
              {hasAiTextAutoGeneratedFields &&
                AI_AUTO_GENERATED_FIELDS_ALL.map(field => (
                  <Fragment key={field}>
                    {renderSubStatus(
                      aiTextAutoGeneratedFields.includes(field) ? 'checked' : 'optional',
                      field
                    )}
                  </Fragment>
                ))}
              Comma-separated fields to auto-generate when uploading photos. Accepted values: title,
              caption, tags, description, all, or none (default: {'"title,tags,semantic"'}):
              {renderEnvVars(['AI_TEXT_AUTO_GENERATED_FIELDS'])}
            </ChecklistRow>
            <ChecklistRow
              title={
                hasRedisStorage && isAnalyzingConfiguration
            ...
            >
              ...
              Create Upstash Redis store from storage tab on Vercel dashboard and connect to this
              project to enable rate limiting
            </ChecklistRow>
            <ChecklistRow
          ...
          <ChecklistGroup title="Performance" icon={<RiSpeedMiniLine size={18} />} optional>
            <ChecklistRow title="Static optimization" status={isStaticallyOptimized} optional>
              Set environment variable to {'"1"'} to make site more responsive by enabling static
            ...
            <ChecklistRow
              ...
              optional
            >
              Set environment variable to {'"1"'} to prevent image uploads being compressed before
              storing:
            ...
            <ChecklistRow
              ...
              optional
            >
              Set environment variable from {'"1-100"'} to control the quality of large photos (
          ...
          <ChecklistGroup title="Visual" icon={<PiPaintBrushHousehold size={19} />} optional>
            <ChecklistRow
              title={`Default theme: ${defaultTheme}`}
              status={hasDefaultTheme}
              optional
            >
              {"Set environment variable to 'light' or 'dark'"} to configure initial theme (defaults
              to {"'system'"}):
              {renderEnvVars(['NEXT_PUBLIC_DEFAULT_THEME'])}
            </ChecklistRow>
            <ChecklistRow title="Photo matting" status={arePhotosMatted} optional>
              Set environment variable to {'"1"'} to constrain the size of each photo, and display a
              surrounding border:
              <div className="pt-1 flex flex-col gap-1">
          ...
          <ChecklistGroup title="Display" icon={<BiHide size={18} />} optional>
            <ChecklistRow title="Category visibility" status={hasCategoryVisibility} optional>
              <div className="my-1">
                ...
                {getHiddenCategories(categoryVisibility).map(category => (
                  <Fragment key={category}>
                    {renderSubStatus(
                      'optional',
              ...
              Configure order and visibility of categories (seen in grid sidebar and CMD-K results)
              by storing comma-separated values (default: {`"${DEFAULT_CATEGORY_KEYS.join(',')}"`}):
            ...
            <ChecklistRow
              ...
              optional
            >
              Set environment variable to {'"1"'} to hide keyboard shortcut tooltips in areas like
              the main nav, and previous/next photo links:
            ...
            <ChecklistRow title="Show zoom controls" status={showZoomControls} optional>
              Set environment variable to {'"1"'} to hide fullscreen photo zoom controls:
              {renderEnvVars(['NEXT_PUBLIC_HIDE_ZOOM_CONTROLS'])}
            </ChecklistRow>
            <ChecklistRow title="Show taken at time" status={showTakenAtTimeHidden} optional>
            ...
            <ChecklistRow title="Show social" status={showSocial} optional>
              Set environment variable to {'"1"'} to hide X (formerly Twitter) button from share
              modal:
          ...
          <ChecklistGroup title="Grid" icon={<IoMdGrid size={17} />} optional>
            ...
            <ChecklistRow
              ...
              optional
            >
              Set environment variable to {'"1"'} to ensure large thumbnails on photo grid views (if
              not configured, density is based on aspect ratio):
          ...
          >
            ...
            <ChecklistRow title="Legacy OG text alignment" status={isOgTextBottomAligned} optional>
              Set environment variable to {'"BOTTOM"'} to keep OG image text bottom aligned (default
              is {'"top"'}):
              {renderEnvVars(['NEXT_PUBLIC_OG_TEXT_ALIGNMENT'])}
            </ChecklistRow>
          </ChecklistGroup>
          {areInternalToolsEnabled && (
            <ChecklistGroup title="Internal" icon={<CgDebug size={16} />} optional>
              <ChecklistRow title="Debug tools" status={areAdminDebugToolsEnabled} optional>
                Set environment variable to {'"1"'} to temporarily enable features like photo
                matting, baseline grid, etc.:
                {renderEnvVars(['ADMIN_DEBUG_TOOLS'])}
              </ChecklistRow>
              <ChecklistRow title="DB optimize" status={isAdminDbOptimizeEnabled} optional>
                Set environment variable to {'"1"'} to prevent homepages from seeding infinite
                scroll on load:
                {renderEnvVars(['ADMIN_DB_OPTIMIZE'])}
              </ChecklistRow>
              <ChecklistRow title="SQL debugging" status={isAdminSqlDebugEnabled} optional>
      ...
      <div className="pl-11 pr-2 sm:pr-11 mt-4 md:mt-7">
        <div>Changes to environment variables require a redeploy or reboot of local dev server</div>
        {!simplifiedView && (
          <div className="text-dim before:content-['—']">
            <div className="flex whitespace-nowrap">
              <span className="font-bold">Domain</span>
              &nbsp;&nbsp;
              <span className="w-full flex overflow-x-auto">{baseUrl || 'Not Defined'}</span>
            </div>
          </div>
        )}
      </div>
