...
export default function PhotoSyncButton({
  ...
  useScrollIntoView({
    ref,
    shouldScrollIntoView: isSyncingExternal && shouldScrollIntoViewOnExternalSync
  });
  ...
  return (
    <Tooltip content="Regenerate photo data">
      <LoaderButton
        ...
        icon={<IconGrSync className="translate-y-[0.5px] translate-x-[0.5px]" />}
        onClick={() => {
          if (!shouldConfirm || window.confirm(syncPhotoConfirmText(photo, hasAiTextGeneration))) {
            setIsSyncing(true);
            syncPhotoAction(photo.id)
              .then(() => {
                onSyncComplete?.();
                if (shouldToast) {
                  toastSuccess(photo.title ? `"${photo.title}" data synced` : 'Data synced');
                }
              })
