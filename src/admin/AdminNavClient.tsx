    ...
    <AppGrid
      contentMain={
        <div className="space-y-4">
          <div className={clsx('flex gap-2 pb-3', 'border-b border-gray-200 dark:border-gray-800')}>
            <MaskedScroll className="grow -mx-1 flex gap-0.5 md:gap-1.5" direction="horizontal">
            ...
            <LinkWithIconLoader
              href={includeInsights ? PATH_ADMIN_INSIGHTS : PATH_ADMIN_CONFIGURATION}
              className={clsx(
                isPathAdminInfo(pathname) ? 'font-bold' : 'text-dim',
          ...
          {isPathAdminInfo(pathname) && <AdminInfoNav {...{ includeInsights }} />}
        </div>
      }
    />
  );
