...
export default async function AdminRecipeTable({ recipes }: { recipes: Recipes }) {
    ...
    <AdminTable>
      {sortRecipes(recipes).map(({ recipe, count }) => (
        <Fragment key={recipe}>
          ...
          <div className={clsx('flex flex-nowrap', 'gap-2 sm:gap-3 items-center')}>
            <EditButton path={pathForAdminRecipeEdit(recipe)} />
            <FormWithConfirm
              action={deletePhotoRecipeGloballyAction}
              confirmText={`Are you sure you want to remove "${formatRecipe(recipe)}" from ${photoQuantityText(count, appText, false, false).toLowerCase()}?`}
            >
