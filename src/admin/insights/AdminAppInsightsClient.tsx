...
const readmeAnchor = (anchor: string) => (
  <AdminLink href={`${TEMPLATE_REPO_URL_README}#${anchor}`}>README/{anchor}</AdminLink>
);

const renderLabeledEnvVar = (label: string, envVar: string, value?: string) => (
  <div className="flex flex-col gap-0.5">
    <span className="text-xs uppercase font-medium tracking-wider">{label}</span>
    <EnvVar variable={envVar} value={value} />
  </div>
);

const renderHighlightText = (text: string, color: 'blue' | 'yellow' = 'blue', truncate = true) => (
  <span
    className={clsx(
...
export default function AdminAppInsightsClient({
  ...
  const branchLink = (
    <a className="truncate" href={codeMeta?.urlBranch} target="blank">
      {codeMeta?.branch ?? TEMPLATE_REPO_BRANCH}
    </a>
  );

  const renderTooltipContent = (content: ReactNode) => (
    <Tooltip content={content} classNameTrigger="ml-1.5" supportMobile />
  );
    ...
    <ScoreCardContainer>
        ...
        <>
          <ScoreCard title="Source code">
            {(codeMeta?.didError || debug) && (
              <ScoreCardRow
                icon={<IoSyncCircle size={18} className={TEXT_COLOR_WARNING} />}
                content={
                  <>
                    <span>Could not analyze source code</span>
                    {renderTooltipContent('Could not connect to GitHub API. Try refreshing.')}
                  </>
                }
              />
            )}
            {((!codeMeta?.didError && noFork) || debug) && (
              <ScoreCardRow
                icon={<FaCircleInfo size={15} className="text-blue-500 translate-y-[1px]" />}
                content="This template is not forked"
                expandContent={
                  <>
                    <AdminLink href={TEMPLATE_REPO_URL_FORK}>Fork original template</AdminLink> to
                    receive the latest fixes and features. Additional instructions in{' '}
                    {readmeAnchor('receiving-updates')}.
                  </>
                }
              />
            )}
            {((!codeMeta?.didError && forkBehind) || debug) && (
              <ScoreCardRow
                icon={<IoSyncCircle size={18} className="text-blue-500" />}
                content={
                  <>
                    This fork is{' '}
                    {renderHighlightText(
                ...
                expandContent={
                  <>
                    <AdminLink href={codeMeta?.urlRepo ?? ''}>Sync your fork</AdminLink> to receive
              ...
              content={
                <div className="flex flex-wrap gap-x-4 gap-y-1 overflow-auto">
                  <div className="flex items-center gap-1 *:whitespace-nowrap">
                    <a href={codeMeta?.urlOwner} target="blank">
                      {codeMeta?.owner ?? TEMPLATE_REPO_OWNER}
                    </a>
                    <div>/</div>
                    <a href={codeMeta?.urlRepo} target="blank">
            ...
            />
            <ScoreCardRow
              icon={<BiGitCommit size={18} className="translate-y-[-0.5px]" />}
              content={
                <a href={codeMeta?.urlCommit} target="blank" className="flex items-center gap-2">
      ...
      <ScoreCard title="Template recommendations">
        {hasTemplateRecommendations(insights) || debug ? (
          <>
            {(noAiRateLimiting || debug) && (
              <ScoreCardRow
                icon={
                  <PiWarningBold
                    size={17}
                    className={clsx('translate-x-[0.5px]', TEXT_COLOR_WARNING)}
                  />
                }
                content={isExpanded =>
                  renderHighlightText('Enable AI rate limiting', 'yellow', !isExpanded)
                }
                expandContent={
                  <>
                    Create Upstash Redis store from storage tab on Vercel dashboard and link to this
                    project to prevent abuse by enabling rate limiting.
                  </>
                }
              />
            )}
            {(noConfiguredDomain || debug) && (
              <ScoreCardRow
                ...
                content={isExpanded =>
                  renderHighlightText('Configure domain', 'yellow', !isExpanded)
                }
                expandContent={
                  <>
                    Not setting an explicit domain may cause certain features to behave
                    unexpectedly. Domains are stored in{' '}
                    <EnvVar variable="NEXT_PUBLIC_DOMAIN" trailingContent="." />
                  </>
                }
              />
            )}
            {(noConfiguredMeta || debug) && (
              <ScoreCardRow
                icon={
                ...
                expandContent={
                  <>
                    Configure site title (visible in search results and browser tab) and site
                    description (visible in search results):{' '}
                    <div className="flex flex-col gap-y-4 mt-3">
                      {renderLabeledEnvVar('Site title', 'NEXT_PUBLIC_META_TITLE')}
                      {renderLabeledEnvVar('Site description', 'NEXT_PUBLIC_META_DESCRIPTION')}
                    </div>
                  </>
                }
              />
            ...
            {(noStaticOptimization || debug) && (
              <ScoreCardRow
                icon={
                  <RiSpeedMiniLine size={19} className="translate-x-[1px] translate-y-[-1.5px]" />
                }
                ...
                expandContent={
                  <>
                    ...
                    <div className="flex flex-col gap-y-4 mt-3">
                      ...
                      <span>See {readmeAnchor('performance')} for cost implications.</span>
                    </div>
                  </>
                }
              />
            ...
            {(noAi || debug) && (
              <ScoreCardRow
                ...
                expandContent={
                  <>
                    Enable automatic AI text generation by setting{' '}
                    <EnvVar variable="OPENAI_SECRET_KEY" trailingContent="." /> Further instruction
                    and cost considerations in {readmeAnchor('ai-text-generation')}.
                  </>
                }
              />
            )}
            {(photoMatting || debug) && (
              <ScoreCardRow
                ...
                expandContent={
                  <>
                    Enable photo matting to make portrait and landscape photos appear more
                    consistent{' '}
                    <EnvVar variable="NEXT_PUBLIC_MATTE_PHOTOS" value="1" trailingContent="." />
                  </>
                }
              />
            )}
            {(camerasFirst || debug) && (
              <ScoreCardRow
                icon={<HiMiniArrowsUpDown size={17} className="translate-x-[-1px]" />}
                content="Move cameras above tags in sidebar"
                expandContent={
                  <>
                    Now that you have more than a few tags, consider showing cameras first in the
                    sidebar by setting{' '}
                    <EnvVar
            ...
            {(gridFirst || debug) && (
              <ScoreCardRow
                ...
                expandContent={
                  <>
                    Now that you have enough photos, consider switching your default view to grid by
                    setting{' '}
                    <EnvVar variable="NEXT_PUBLIC_GRID_HOMEPAGE" value="1" trailingContent="." />
                  </>
                }
              />
            )}
          ...
          <AdminEmptyState includeContainer={false}>Nothing to report!</AdminEmptyState>
        )}
      </ScoreCard>
      <ScoreCard title="Library Stats">
        {(photosNeedSync || debug) && (
          <ScoreCardRow
            icon={
              <LiaBroomSolid size={19} className={clsx('translate-y-[-2px]', TEXT_COLOR_BLUE)} />
            }
            content={
              <>
                {renderHighlightText(
                  pluralize(photosCountNeedSync || DEBUG_PHOTOS_NEED_SYNC_COUNT, 'photo'),
                  'blue'
                )}{' '}
                with updates
                {renderTooltipContent(<>Missing data or AI&#8209;generated text</>)}
              </>
            }
        ...
        <ScoreCardRow
          icon={<IconPhoto size={15} className="translate-y-[0.5px]" />}
          content={
            <>
          ...
          switch (category) {
            ...
            case 'tags':
              return (
                <ScoreCardRow
                  key={category}
                  icon={<IconTag size={15} className="translate-x-[1px] translate-y-[1px]" />}
            ...
            case 'recipes':
              return recipesCount > 0 ? (
                <ScoreCardRow
                  key={category}
                  icon={
                    <IconRecipe size={18} className="translate-x-[0.5px] translate-y-[-0.5px]" />
                  }
        ...
        {descriptionWithSpaces && (
          <ScoreCardRow
            icon={<FaRegCalendar size={13} className="translate-y-[1.5px]" />}
