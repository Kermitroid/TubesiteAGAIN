...
const AdminAppInsightCode = ['noFork', 'forkBehind'] as const;
...
const _INSIGHTS_LIBRARY = ['photosNeedSync'] as const;
...
export const getSignificantInsights = ({
  ...
  const { isAiTextGenerationEnabled, hasRedisStorage, hasDomain } = APP_CONFIGURATION;
...
export const indicatorStatusForSignificantInsights = ({
  ...
  const { forkBehind, noAiRateLimiting, noConfiguredDomain, photosNeedSync } = insights;
...
export const getAllInsights = ({
  ...
  noConfiguredMeta: !IS_META_TITLE_CONFIGURED || !IS_META_DESCRIPTION_CONFIGURED,
  photoMatting: photosCountPortrait > 0 && !MATTE_PHOTOS,
  camerasFirst: tagsCount > TAG_COUNT_THRESHOLD && CATEGORY_VISIBILITY[0] !== 'cameras',
