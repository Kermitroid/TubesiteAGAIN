    ...
    <AdminTable>
      {photos.map((photo, index) => (
        <Fragment key={photo.id}>
          ...
          />
          <div className={clsx('flex flex-col lg:flex-row min-w-0', opacityForPhotoId(photo.id))}>
            <Link
              key={photo.id}
              ...
              prefetch={false}
            >
              <span className={clsx('flex w-full gap-1.5', photo.hidden && 'text-dim')}>
                <span className="truncate">{titleForPhoto(photo, false)}</span>
                {photo.hidden && (
                  <span>
                    <IconHidden className="inline translate-y-[-0.5px]" size={16} />
                  </span>
                )}
              </span>
            ...
            <div
              className={clsx('flex min-w-0 gap-1.5 w-full', 'lg:w-[50%] uppercase', 'text-dim')}
            >
              <PhotoDate {...{ photo, dateType, timezone }} className="truncate" />
          ...
          <div className={clsx('flex flex-nowrap', 'gap-2 items-center')}>
