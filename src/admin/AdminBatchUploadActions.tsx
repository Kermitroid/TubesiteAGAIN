        ...
        <div className="w-full space-y-4 py-1">
          ...
          {showBulkSettings && !actionErrorMessage && (
            <div className="space-y-4 mb-6">
              ...
              <div className="flex gap-8">
                <FieldsetFavs value={favorite} onChange={setFavorite} readOnly={isAdding} />
                <FieldsetHidden value={hidden} onChange={setHidden} readOnly={isAdding} />
              </div>
            </div>
          )}
          <div className="space-y-2">
            <ProgressButton
              ...
              isLoading={isAdding}
              disabled={Boolean(tagErrorMessage) || isAddingComplete || isDeleting}
              icon={
                isAddingComplete ? (
                  <BiCheckCircle size={18} className="translate-x-[1px]" />
                ) : (
                  <BiImageAdd size={18} className="translate-x-[1px] translate-y-[2px]" />
                )
              }
              onClick={async () => {
                if (confirm(`Are you sure you want to add all ${storageUrls.length} uploads?`)) {
                  ...
                  try {
                    while (uploadsToAdd.length > 0) {
                      const nextBatch = uploadsToAdd.splice(0, UPLOAD_BATCH_SIZE);
                      await addUploadUrls(nextBatch, uploadsToAdd.length === 0);
                    }
