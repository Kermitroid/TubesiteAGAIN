...
const CATEGORY_KEYS = ['cameras', 'lenses', 'tags', 'recipes', 'films', 'focal-lengths'] as const;
...
export const getOrderedCategoriesFromString = (categories?: string): CategoryKeys =>
  categories
    ? categories
        .split(',')
...
export const sortCategoryByCount = (a: { count: number }, b: { count: number }) =>
  b.count - a.count;

export const sortCategoriesByCount = <T extends { count: number }>(categories: T[]) =>
  categories.sort(sortCategoryByCount);

const convertCategoryKeysToCategoryNames = (
  categoryKeys: CategoryKeys
): (keyof PhotoSetCategories)[] => {
  return categoryKeys.map(key => {
    return key === 'focal-lengths' ? 'focalLengths' : key;
  });
};

export const getCategoryItemsCount = (categoryKeys: CategoryKeys, categories: PhotoSetCategories) =>
  convertCategoryKeysToCategoryNames(categoryKeys).reduce(
    (acc, key) => acc + (categories[key]?.length ?? 0),
    0
  );
...
export const getCategoriesWithItemsCount = (
  ...
  convertCategoryKeysToCategoryNames(categoryKeys).reduce(
    (acc, key) => ((categories[key]?.length ?? 0) > 0 ? acc + 1 : acc),
    0
  );
