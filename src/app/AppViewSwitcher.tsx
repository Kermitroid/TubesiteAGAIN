...
import { GRID_HOMEPAGE_ENABLED, SHOW_KEYBOARD_SHORTCUT_TOOLTIPS } from './config';
...
export default function AppViewSwitcher({
  ...
  const { isUserSignedIn, isUserSignedInEager, setIsCommandKOpen } = useAppState();
      ...
      if (!e.metaKey) {
        switch (e.key.toLocaleUpperCase()) {
          case KEY_COMMANDS.feed:
            if (pathname !== PATH_FEED_INFERRED) {
              refHrefFeed.current?.click();
            }
            break;
  ...
  return (
    <div className={clsx('flex gap-1 sm:gap-2', className)}>
      <Switcher>
        ...
        {isUserSignedIn && (
          <SwitcherItem
            icon={<AdminAppMenu isOpen={isAdminMenuOpen} setIsOpen={setIsAdminMenuOpen} />}
            tooltip={{
              ...(!isAdminMenuOpen &&
                SHOW_KEYBOARD_SHORTCUT_TOOLTIPS && {
                  content: appText.nav.admin,
