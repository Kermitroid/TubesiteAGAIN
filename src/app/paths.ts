...
export const pathForAdminRecipeEdit = (recipe: string) => `${PATH_ADMIN_RECIPES}/${recipe}/${EDIT}`;
...
export const pathForFilm = (film: string) => `${PREFIX_FILM}/${film}`;
...
export const pathForRecipe = (recipe: string) => `${PREFIX_RECIPE}/${recipe}`;

// Absolute paths
export const ABSOLUTE_PATH_FOR_HOME_IMAGE = `${getBaseUrl()}/home-image`;

export const absolutePathForPhoto = (params: PhotoPathParams, share?: boolean) =>
...
export const absolutePathForLensImage = (lens: Lens) => `${absolutePathForLens(lens)}/image`;

export const absolutePathForFilmImage = (film: string) => `${absolutePathForFilm(film)}/image`;
...
// film/[film]
export const isPathFilm = (pathname = '') => new RegExp(`^${PREFIX_FILM}/[^/]+/?$`).test(pathname);
...
export const isPathTopLevel = (pathname?: string) =>
  isPathRoot(pathname) || isPathGrid(pathname) || isPathFeed(pathname);
...
export const isPathAdminInfo = (pathname?: string) =>
  isPathAdminInsights(pathname) || isPathAdminConfiguration(pathname);
...
export const getPathComponents = (
  ...
  const photoIdFromTag = pathname.match(new RegExp(`^${PREFIX_TAG}/[^/]+/([^/]+)`))?.[1];
  const photoIdFromCamera = pathname.match(
    new RegExp(`^${PREFIX_CAMERA}/[^/]+/[^/]+/([^/]+)`)
  )?.[1];
  const photoIdFromFilm = pathname.match(new RegExp(`^${PREFIX_FILM}/[^/]+/([^/]+)`))?.[1];
  ...
  const film = pathname.match(new RegExp(`^${PREFIX_FILM}/([^/]+)`))?.[1] as string;
...
export const getEscapePath = (pathname?: string) => {
  const { photoId, tag, camera, film, focal } = getPathComponents(pathname);
