...
export default function useStickyNav(ref: RefObject<HTMLElement | null>, isEnabled = true) {
  ...
  const isNavSticky = isEnabled && (hasScrolledPastNav || scrollDirection === 'up');

  const shouldHideStickyNav = isNavSticky && scrollDirection === 'down';

  const shouldAnimateStickyNav =
    isNavSticky && (scrollY > navHeight * 2 || scrollDirection === 'up');

  const classNames = useMemo(
    () => ({
      classNameStickyContainer: clsx(isNavSticky && 'sticky top-0 z-10 pointer-events-none'),
