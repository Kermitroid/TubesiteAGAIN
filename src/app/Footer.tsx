...
export default function Footer() {
  ...
  const { userEmail, userEmailEager, isCheckingAuth, clearAuthStateAndRedirectIfNecessary } =
    useAppState();
                  ...
                  >
                    <div className="flex gap-x-3 xs:gap-x-4 grow flex-wrap">
                      {userEmail || userEmailEager ? (
                        <>
                          <div className="truncate max-w-full">{userEmail || userEmailEager}</div>
                          <form
                            action={() =>
                              signOutAction().then(clearAuthStateAndRedirectIfNecessary)
                            }
                          >
                        ...
                        <Link href={PATH_ADMIN_PHOTOS}>{appText.nav.admin}</Link>
                      )}
                    </div>
