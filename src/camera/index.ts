...
export const formatCameraParams = ({ make, model }: { make: string; model: string }): Camera => ({
  make: parameterize(make),
  model: parameterize(model)
});

export const sortCamerasWithCount = (a: CameraWithMeta, b: CameraWithMeta) => {
...
export const formatCameraText = (
  ...
  const doesModelStartWithMake =
    makeSimple && model.toLocaleLowerCase().startsWith(makeSimple.toLocaleLowerCase());
  switch (length) {
    case 'long':
      return `${make} ${model}`;
    case 'medium':
      return doesModelStartWithMake || isCameraMakeApple(make) ? model : `${make} ${model}`;
    case 'short':
      model = doesModelStartWithMake ? model.replace(makeSimple, '').trim() : model;
      if (model.includes('iPhone') && model.length > 9) {
