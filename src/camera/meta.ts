...
import { absolutePathForCamera, absolutePathForCameraImage } from '@/app/paths';
...
export const titleForCamera = (
  ...
  [
    appText.category.cameraTitle(formatCameraText(cameraFromPhoto(photos[0], camera))),
    photoQuantityText(explicitCount ?? photos.length, appText)
  ].join(' ');

export const shareTextForCamera = (camera: Camera, photos: Photo[], appText: AppTextState) =>
  appText.category.cameraShare(formatCameraText(cameraFromPhoto(photos[0], camera)));
...
export const generateMetaForCamera = (
  ...
  description: descriptionForCameraPhotos(photos, appText, true, explicitCount, explicitDateRange),
