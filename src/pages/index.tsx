  ...
  const share = useCallback(() => {
    ...
    self
      ...
      .then(res => {
        ...
        return new Promise(async (_, reject) => reject(new Error(await res.text())));
      })
      .then(({ snapshotId }) => {
    ...
    <>
      ...
      {currentError && (
        <ErrorDialog onExit={onClearError}>
          <p>An error occurred while saving your snapshot. Please try again in a bit.</p>
          <pre>{currentError.message}</pre>
        </ErrorDialog>
      )}
      {currentSnapshotId && (
        <ShareLinkDialog snapshotId={currentSnapshotId} onExit={clearSnapshot} />
      )}
        ...
        {props.hasError ? (
          <>
            ...
            <div className="explanation" style={{ textAlign: 'center' }}>
              <p>
                The production website is <strong>still available</strong> and this does not affect
                other users.
              </p>
            </div>
            <hr />
      ...
      {isEdit ? (
        <>
          <Snapshot onCancel={toggleEdit} onShare={share} isSharing={isSharingView} />
        </>
      ) : (
    ...
    <>
      ...
      <div className="features">
        <div className="feature">
          <Malleable id="feature-1-emoji" as="div" isActive={isEdit} edits={edits}>
            ⚡
          </Malleable>
          <Malleable id="feature-1-text" as="h4" isActive={isEdit} edits={edits}>
            Blazing fast
          </Malleable>
        </div>
        <div className="feature">
          <Malleable id="feature-2-emoji" as="div" isActive={isEdit} edits={edits}>
            📡
          </Malleable>
          <Malleable id="feature-2-text" as="h4" isActive={isEdit} edits={edits}>
            Always available
          </Malleable>
        </div>
        <div className="feature">
          <Malleable id="feature-3-emoji" as="div" isActive={isEdit} edits={edits}>
            🏎
          </Malleable>
          <Malleable id="feature-3-text" as="h4" isActive={isEdit} edits={edits}>
            Lighthouse 100
          </Malleable>
        </div>
      </div>
      ...
      <div className="explanation">
        <div className="p">
          <Malleable id="explanation-1-inspect" as="span" isActive={isEdit} edits={edits}>
          ...
          <br />
          <Malleable id="explanation-1-pre-curl" as="pre" isActive={isEdit} edits={edits}>
        ...
        <Malleable id="explanation-2" isActive={isEdit} edits={edits}>
          When people visit this site, the response always comes instantly from their{' '}
          <a
            target="_blank"
            rel="noopener noreferrer"
            href="https://vercel.com/docs/concepts/edge-network/overview"
        ...
        <Malleable id="explanation-3" isActive={isEdit} edits={edits}>
          Unlike traditional static solutions, however, you can generate previews of edits that you
          can share with anyone you choose. To try it out, click the edit icon on the bottom right
          and edit the content. When you're done, click the share icon on the bottom right to
          generate a shareable preview URL.
        </Malleable>
        <Malleable id="explanation-4" isActive={isEdit} edits={edits}>
          SSG and Preview Mode make Next.js the most optimal framework to integrate into your
          Headless CMS workflow. Learn more about the preview mode on{' '}
          <a
            target="_blank"
            rel="noopener noreferrer"
