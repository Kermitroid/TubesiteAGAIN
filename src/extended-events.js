...
class ExtendedEvents extends EventEmitter {
  constructor(player, initOptions = {}) {
    ...
    const timeupdate = event => {
      ...
      if (this.events.percentsplayed) {
        this.events.percentsplayed.percents.forEach(percent => {
          if (
            playedAtPercentage(currentTime, duration, percent) &&
            _percentsTracked.indexOf(percent) === -1
          ) {
      ...
      if (this.events.timeplayed) {
        ...
        const times = timeplayed.interval
          ? [Math.floor(currentTime / timeplayed.interval) * timeplayed.interval]
          : timeplayed.times;
    ...
    if (
      this.events.percentsplayed ||
      this.events.timeplayed ||
      this.events.seek ||
      this.events.totaltimeplayed
    ) {
...
const playedAtPercentage = (
  currentTime,
  duration,
  percentageCheckpoint,
  graceRangeSeconds = 0.5
) => {
...
const playedAtTime = (currentTime, checkpoint, graceRangeSeconds = 0.5) =>
  currentTime <= checkpoint + graceRangeSeconds && currentTime >= checkpoint - graceRangeSeconds;
