...
export const getLensFromParams = async (params: Promise<Lens>): Promise<Lens> => {
  const { make, model } = await params;
  return make === MISSING_FIELD ? { model } : { make, model };
};
...
export const getLensPhotoFromParams = async (
  ...
  return make === MISSING_FIELD ? { model, photoId } : { make, model, photoId };
};

export const safelyGenerateLensStaticParams = (lenses: Lenses) =>
...
export const formatLensParams = ({ make, model }: { make?: string; model: string }): Lens => ({
  make: make ? parameterize(make) : undefined,
  model: parameterize(model)
});

export const sortLensesWithCount = (a: LensWithMeta, b: LensWithMeta) => {
...
export const formatLensText = (
  ...
  const doesModelStartWithMake =
    makeSimple && modelRaw.toLocaleLowerCase().startsWith(makeSimple.toLocaleLowerCase());
