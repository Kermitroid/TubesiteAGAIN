...
import {
  ...
  isRawUrl,
  ERROR_CODE
} from './plugins/cloudinary/common';
...
class VideoPlayer extends Utils.mixin(Eventable) {
  ...
  _setVideoJsListeners(ready) {
    this.videojs.on(PLAYER_EVENT.ERROR, () => {
      ...
      if (error) {
        ...
        if (
          [isCorrupted, error.code === 4].includes(true) &&
          [SOURCE_TYPE.AUDIO, SOURCE_TYPE.VIDEO].includes(type)
        ) {
  ...
  _initHighlightsGraph() {
    if (this.playerOptions.aiHighlightsGraph) {
      this.videojs.on(PLAYER_EVENT.CLD_SOURCE_CHANGED, (e, { source }) => {
        ...
        const inputTransformations = pick(source.transformation(), [
          'start_offset',
          'end_offset',
          'duration'
        ]);
        ...
        const aiHighlightsGraphSrc = source
          .config()
  ...
  _initCloudinaryAnalytics() {
    ...
    if (cloudinaryAnalyticsOptionEnabled) {
      ...
      const options = isObject(this.playerOptions.cloudinaryAnalytics)
        ? this.playerOptions.cloudinaryAnalytics
        : {};
  ...
  reTryVideoStateUntilAvailable(
    maxNumberOfCalls = Number.POSITIVE_INFINITY,
    timeout = RETRY_DEFAULT_TIMEOUT
  ) {
    ...
    if (!isVideoInReadyState(this.videojs.readyState())) {
      if (this.reTryVideoStateRetriesCount < maxNumberOfCalls) {
        ...
        this.reTryVideoStateTimeoutId = this.videojs.setTimeout(
          () => this.reTryVideoStateUntilAvailable(maxNumberOfCalls, timeout),
          timeout
        );
      } else {
  ...
  _setExtendedEvents() {
    ...
    events.push(
      ...[PLAYER_EVENT.SEEK, PLAYER_EVENT.MUTE, PLAYER_EVENT.UNMUTE, PLAYER_EVENT.QUALITY_CHANGED]
    );
  ...
  playlist(sources, options = {}) {
    ...
    return this.videojs.cloudinary.playlist
      ? this.videojs.cloudinary.playlist(sources, options)
      : this;
  }
