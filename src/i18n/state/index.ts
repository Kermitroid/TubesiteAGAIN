...
export const generateAppTextState = (i18n: I18N) => {
  return {
    ...
    category: {
      ...i18n.category,
      cameraTitle: (camera: string) => i18n.category.cameraTitle.replace('{{camera}}', camera),
      cameraShare: (camera: string) => i18n.category.cameraShare.replace('{{camera}}', camera),
      taggedPhrase: (tag: string) => i18n.category.taggedPhrase.replace('{{tag}}', tag),
      recipeShare: (recipe: string) => i18n.category.recipeShare.replace('{{recipe}}', recipe),
      filmShare: (film: string) => i18n.category.filmShare.replace('{{film}}', film),
    ...
    misc: {
      ...i18n.misc,
      copyPhrase: (label: string) => i18n.misc.copyPhrase.replace('{{label}}', label)
    },
    utility: {
      ...i18n.utility,
