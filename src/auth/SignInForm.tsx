...
import { KEY_CALLBACK_URL, KEY_CREDENTIALS_SIGN_IN_ERROR, KEY_CREDENTIALS_SUCCESS } from '.';
...
export default function SignInForm({
  ...
  return (
    <Container
      className={clsx('w-[calc(100vw-1.5rem)] sm:w-[min(360px,90vw)]', 'px-6 py-5', className)}
    >
      {includeTitle && (
        <h1
          className={clsx('flex gap-3 items-center justify-center', 'self-start text-2xl', 'mb-6')}
        >
          <IconLock className="text-main translate-y-[0.5px]" />
          <span className="text-main">{appText.auth.signIn}</span>
        </h1>
      )}
      <form action={action} className="w-full">
        <div className="space-y-5 w-full -translate-y-0.5">
          {response === KEY_CREDENTIALS_SIGN_IN_ERROR && (
            <ErrorNote>{appText.auth.invalidEmailPassword}</ErrorNote>
          )}
