...
import { absolutePathForTag, absolutePathForTagImage, getPathComponents } from '@/app/paths';
import {
  capitalizeWords,
...
) => [formatTag(tag), photoQuantityText(explicitCount ?? photos.length, appText)].join(' ');

export const shareTextForTag = (tag: string, appText: AppTextState) =>
  isTagFavs(tag) ? appText.category.taggedFavs : appText.category.taggedPhrase(formatTag(tag));

export const sortTagsArray = (tags: string[], tagToExclude?: string) =>
  tags.filter(tag => tag !== tagToExclude).sort((a, b) => (isTagFavs(a) ? -1 : a.localeCompare(b)));

export const sortTags = (tags: Tags, tagToExclude?: string) =>
  tags
    .filter(({ tag }) => tag !== tagToExclude)
    .sort(({ tag: a }, { tag: b }) => (isTagFavs(a) ? -1 : isTagFavs(b) ? 1 : a.localeCompare(b)));

export const sortTagsByCount = (tags: Tags, tagToExclude?: string) =>
  tags
    .filter(({ tag }) => tag !== tagToExclude)
    .sort(({ tag: tagA, count: countA }, { tag: tagB, count: countB }) =>
      isTagFavs(tagA) ? -1 : isTagFavs(tagB) ? 1 : countB - countA
    );

export const sortTagsWithoutFavs = (tags: string[]) => sortTagsArray(tags, TAG_FAVS);

export const sortTagsObjectWithoutFavs = (tags: Tags) => sortTags(tags, TAG_FAVS);
...
export const generateMetaForTag = (
  ...
  description: descriptionForTaggedPhotos(photos, appText, true, explicitCount, explicitDateRange),
...
export const addHiddenToTags = (tags: Tags, countHidden = 0, lastModifiedHidden = new Date()) =>
  countHidden > 0
    ? tags
...
export const convertTagsForForm = (tags: Tags = [], appText: AppTextState) =>
  sortTagsObjectWithoutFavs(tags).map(({ tag, count }) => ({
    value: tag,
    annotation: formatCount(count),
    annotationAria: formatCountDescriptive(count, appText.category.taggedPhotos)
  }));
