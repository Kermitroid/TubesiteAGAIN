...
export const formatDate = ({
  ...
  let formatString = !hideTime ? DATE_STRING_FORMAT_LONG : DATE_STRING_FORMAT_SHORT;
  ...
  switch (length) {
    ...
    case 'short':
      ...
      break;
    case 'medium':
      formatString = !hideTime ? DATE_STRING_FORMAT_MEDIUM : DATE_STRING_FORMAT_SHORT;
  ...
  return showPlaceholder
    ? placeholderString
    : timezone
      ? formatInTimeZone(date, timezone, formatString, { locale: DATE_FN_LOCALE })
...
export const convertTimestampToNaivePostgresString = (timestamp?: AmbiguousTimestamp) =>
...
export const generateLocalPostgresString = () => formatDateForPostgres(new Date());
...
export const validationMessagePostgresDateString = (date = '') =>
  validatePostgresDateString(date) ? undefined : `Invalid format (${VALIDATION_EXAMPLE_POSTGRES})`;
