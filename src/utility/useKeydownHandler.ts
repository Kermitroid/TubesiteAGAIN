...
export default function useKeydownHandler({
  ...
  const onKeyDown = useCallback(
    (e: KeyboardEvent) => {
      const isKeyValid = !keys || keys.some(key => key.toUpperCase() === e.key?.toUpperCase());
      const isTextEntry =
  ...
  useEffect(() => {
    if (shouldRespondToKeyboardCommands || ignoreShouldRespondToKeyboardCommands) {
      window.addEventListener(LISTENER_KEYDOWN, onKeyDown);
      return () => window.removeEventListener(LISTENER_KEYDOWN, onKeyDown);
    }
  }, [shouldRespondToKeyboardCommands, ignoreShouldRespondToKeyboardCommands, onKeyDown]);
}
