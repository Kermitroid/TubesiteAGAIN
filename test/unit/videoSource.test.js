...
describe('Raw url tests', () => {
  ...
  it('Test raw url with transformations', () => {
    ...
    let url =
      'https://media.castit.biz/castit-dev/video/upload/a_0,c_limit,f_mp4,vc_h264,w_600/cit-qa/Videoback/2020/08/03/269085_358976.mp4';
    let source = new VideoSource(url, sourceDef);
    let srcs = source.generateSources();
    expect(srcs[0].src).toEqual(url);
    expect(srcs[0].type).toEqual('video/mp4');
...
describe('test hasCodec method', () => {
  ...
  describe('test if codec exist in transformations array', () => {
    it('codec exist in transformation array', () => {
      const transformations = [['vc_vp9,q_auto']];
  ...
  describe('should generated codec in source url', () => {
    it('should get default code', () => {
      ...
      expect(srcs[0].src).toEqual(
        'http://res.cloudinary.com/demo/video/upload/f_auto:video/sea_turtle'
      );
    });
    ...
    it('with codec and sourceType', () => {
      ...
      const srcs = source.generateSources();
      expect(srcs[0].src).toEqual(
        'http://res.cloudinary.com/demo/video/upload/vc_vp9/sea_turtle.webm'
      );
    });
    ...
    it('codec with no sourceType', () => {
      ...
      const srcs = source.generateSources();
      expect(srcs[0].src).toEqual(
        'http://res.cloudinary.com/demo/video/upload/vc_vp9/f_auto:video/sea_turtle'
      );
    });
    ...
    it('check if codec has been changed', () => {
      ...
      const srcs = source.generateSources();
      expect(srcs[0].src).toEqual(
        'http://res.cloudinary.com/demo/video/upload/vc_h265/f_auto:video/sea_turtle'
      );
    });
    ...
    it('raw_transformation codec with no sourceType', () => {
      ...
      const srcs = source.generateSources();
      expect(srcs[0].src).toEqual(
        'http://res.cloudinary.com/demo/video/upload/f_auto:video/vc_vp9/sea_turtle'
      );
    });
    ...
    it('check if codec has been change using raw_transformation', () => {
      ...
      const srcs = source.generateSources();
      expect(srcs[0].src).toEqual(
        'http://res.cloudinary.com/demo/video/upload/f_auto:video/h265/sea_turtle'
      );
    });
    ...
    it('check array of transformations without codec', () => {
      ...
      const srcs = source.generateSources();
      expect(srcs[0].src).toEqual(
        'http://res.cloudinary.com/demo/video/upload/w_400/f_auto:video/sea_turtle'
      );
    });
