describe('Colors api tests', () => {
  ...
  it('Test colors', async () => {
    ...
    const playersInf = await page.evaluate(() =>
      players.map(p => {
        const op = {};
    ...
    for (const player of playersInf) {
      ...
      const tooltipColor = rgb2hex(
        await page.$eval(`#${id} .vjs-time-tooltip`, e => getComputedStyle(e).color)
      );
      const progressColor = rgb2hex(
        await page.$eval(`#${id} .vjs-play-progress`, e => getComputedStyle(e).backgroundColor)
      );
      const titleBarColor = rgb2hex(
        await page.$eval(`#${id} .vjs-title-bar`, e => getComputedStyle(e).color)
      );
