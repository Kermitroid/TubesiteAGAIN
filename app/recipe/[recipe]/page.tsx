...
export async function generateMetadata({ params }: RecipeProps): Promise<Metadata> {
  const { recipe: recipeFromParams } = await params;

  const recipe = decodeURIComponent(recipeFromParams);

  const [photos, { count, dateRange }] = await getPhotosRecipeDataCachedCached({
    recipe,
  ...
  const { url, title, description, images } = generateMetaForRecipe(
    recipe,
    photos,
    appText,
    count,
...
export default async function RecipePage({ params }: RecipeProps) {
  ...
  return <RecipeOverview {...{ recipe, photos, count, dateRange }} />;
}
