...
import { PATH_ROOT, absolutePathForPhoto, absolutePathForPhotoImage } from '@/app/paths';
import PhotoDetailPage from '@/photo/PhotoDetailPage';
import { getPhotosMetaCached, getPhotosNearIdCached } from '@/photo/cache';
import { cache } from 'react';

const getPhotosNearIdCachedCached = cache((photoId: string, recipe: string) =>
  getPhotosNearIdCached(photoId, { recipe, limit: RELATED_GRID_PHOTOS_TO_SHOW + 2 })
);
...
export async function generateMetadata({ params }: PhotoRecipeProps): Promise<Metadata> {
...
export default async function PhotoRecipePage({ params }: PhotoRecipeProps) {
  const { photoId, recipe: recipeFromParams } = await params;
