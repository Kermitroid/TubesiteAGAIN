...
import { formatLensParams, getLensPhotoFromParams, lensFromPhoto, LensPhotoProps } from '@/lens';

const getPhotosNearIdCachedCached = cache(
  (photoId: string, make: string | undefined, model: string) =>
...
export async function generateMetadata({ params }: LensPhotoProps): Promise<Metadata> {
...
export default async function PhotoLensPage({ params }: LensPhotoProps) {
  const { photoId, make, model } = await getLensPhotoFromParams(params);
