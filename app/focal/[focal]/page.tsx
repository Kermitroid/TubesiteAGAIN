...
export const generateStaticParams = staticallyGenerateCategoryIfConfigured(
  ...
  focalLengths => focalLengths.map(({ focal }) => ({ focal: focal.toString() }))
);
...
export async function generateMetadata({ params }: FocalLengthProps): Promise<Metadata> {
...
export default async function TagPage({ params }: FocalLengthProps) {
  const { focal: focalString } = await params;
