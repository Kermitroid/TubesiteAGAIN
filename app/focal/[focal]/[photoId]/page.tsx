...
const getPhotosNearIdCachedCached = cache((photoId: string, focal: number) =>
  getPhotosNearIdCached(photoId, { focal, limit: RELATED_GRID_PHOTOS_TO_SHOW + 2 })
);
...
export async function generateMetadata({ params }: PhotoFocalLengthProps): Promise<Metadata> {
...
export default async function PhotoFocalLengthPage({ params }: PhotoFocalLengthProps) {
  const { photoId, focal: focalString } = await params;
