...
import { getUniqueFilmsCached, getUniqueRecipesCached, getUniqueTagsCached } from '@/photo/cache';
...
export default async function UploadPage({ params }: Params) {
  ...
  const isDataMissing = !formDataFromExif || (AI_TEXT_GENERATION_ENABLED && !imageThumbnailBase64);
  ...
  const [uniqueTags, uniqueRecipes, uniqueFilms, recipeTitle] = await Promise.all([
    getUniqueTagsCached(),
    getUniqueRecipesCached(),
    getUniqueFilmsCached(),
    formDataFromExif?.recipeData && formDataFromExif.film
      ? getRecipeTitleForData(formDataFromExif.recipeData, formDataFromExif.film)
      : undefined
  ]);
  ...
  return !isDataMissing ? (
    ...
    <ErrorNote>{error ?? 'Unknown error'}</ErrorNote>
  );
}
