...
import { AI_TEXT_GENERATION_ENABLED, PRESERVE_ORIGINAL_UPLOADS } from '@/app/config';
...
export default async function AdminPhotosPage() {
  ...
  const [photos, photosCount, photosCountNeedsSync, blobPhotoUrls] = await Promise.all([
    getPhotos({
      hidden: 'include',
      sortBy: 'createdAt',
    ...
    getPhotosMetaCached({ hidden: 'include' })
      .then(({ count }) => count)
      .catch(() => 0),
    getPhotosInNeedOfSyncCount().catch(() => 0),
